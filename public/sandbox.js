...
function widgetShim(theme, accent, panelMode) {
  ...
  return (
    ...
    /* v8.7.23 网易云直链播放：api=白名单 weapi 代理，audio=宿主 <audio> 哑播放器，
       sub=状态帧订阅（timeupdate≈4Hz；一次性标志 ended/sysCmd 随 seq 单调） */
    "ne:{api:function(p,d){return new Promise(function(res){var id=++seq;pending[id]={f:res};" +
    "post({type:'widgetApi',op:'neApi',path:String(p||'').slice(0,64),params:JSON.stringify(d||{}).slice(0,2000),reqId:id})})}," +
    "audio:function(a,o){o=o||{};return new Promise(function(res){var id=++seq;pending[id]={f:res};" +
    "post({type:'widgetApi',op:'neAudio',act:String(a||'').slice(0,8),url:String(o.url||'').slice(0,600)," +
    "meta:JSON.stringify(o.meta||{}).slice(0,1000),position:(typeof o.position==='number'&&isFinite(o.position))?o.position:null," +
    ...
    "post({type:'widgetApi',op:'nePub',payload:p2})}}," +
      ...
      window.addEventListener("message", function (ev) {
        ...
        if (d && typeof d === "object" && d.type === "widgetApi") {
          post({
            ...
            /* v8.7.23 网易云通道字段 */
            path: str(d.path, 64),
            params: str(d.params, 2000),
            act: str(d.act, 8),
            url: str(d.url, 600),
            meta: str(d.meta, 1000),
            volume: typeof d.volume === "number" ? d.volume : null,
            payload: str(d.payload, 240000),
          });
        }
      });
      return;
