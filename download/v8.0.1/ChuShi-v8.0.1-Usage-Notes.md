...
3. 「初始」页：`ChuShi-NewTab-v8.0.1.zip` 覆盖旧扩展（或等商店/GitHub Pages 自动更新）。
4. **「初始」页重新导入 `ChuShi-Music-Preset-8.0.1.cshz`**（旧预设里的部件 HTML 是坏的，
...
- ChuShi Music Bridge **8.0.1**（含 hub.dll 8.0.1：防阻塞三律）
- ChuShi Lyric Source 7.0.0（不变）
- 「初始」NewTab **8.0.1**（smtc 客户端容错调优 + 沙箱标准模式修复）
- SMTC 音乐预设 8.0.1（封面尺寸修复 + data-URI 兜底）
