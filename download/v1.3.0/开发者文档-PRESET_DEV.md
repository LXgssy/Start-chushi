...
`cfg` 字段（宿主逐字段白名单夹紧，非法回默认）：`refraction` 折射强度（0–300%，默认 145）、`band` 边缘带宽（8–60%，默认 26）、`frost` 霜化模糊（0–12px，默认 3）、`saturation` 饱和度（100–260%，默认 180）、`brightness` 透亮（80–140%，默认 100）、`dispersion` 边缘色散（布尔，默认 false）、`specular` 镜面高光（布尔，默认 true）、`coverage` 覆盖范围（`"core"` 基础四区 / `"full"` 全部玻璃面，默认 full）。

玻璃面注册表（全屏幕布永不折射——幕布不是玻璃块）：core = `.search-pill`（搜索栏）/ `.cl-dock`（底部栏）/ `.cl-panel`（面板卡片）/ `.glass-card`（⌘K、对话框、右键菜单、开发者文档等全站玻璃卡）；full 另含 `.glass-chip`（天气芯片）。

```js
// 官方「液态玻璃」预设的完整脚本逻辑（examples/液态玻璃预设.json，约 1.8KB）
chushi.settings.define({ title: "液态玻璃", controls: [/* 八项：七项材质 + coverage 覆盖范围 */] });
