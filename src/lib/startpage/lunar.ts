/*
 * 农历计算（1900–2100）
 * 经典查表算法：以 1900-01-31（农历正月初一）为基准。
 * 返回如 「七月初五」 / 「闰二月十四」 的中文字符串。
 */
...
function leapDays(year: number): number {
  ...
  if (LUNAR_INFO[year - 1900] & 0x10000) return 30;
  return 29;
}

/** 某月的农历天数 */
function monthDays(year: number, month: number): number {
  if (LUNAR_INFO[year - 1900] & (0x10000 >> month)) return 30;
  return 29;
}

/** 公历日期 → 「七月初五」 形式的农历文本；超出支持范围返回空串 */
export function getLunarText(date: Date): string {
  ...
  if (offset < 0) {
    offset += month > leap && isLeap ? leapDays(lunarYear) : monthDays(lunarYear, month);
    month--;
  }

  const day = offset + 1;
  const monthName = (isLeap ? "闰" : "") + MONTHS_CN[month - 1] + "月";
  return monthName + DAYS_CN[day - 1];
}
