#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v8.6.34 视觉验证像素判定：MAD(A,C)≈0 磨砂复原 / MAD(A,B) 显著 zen 生效"""
import cv2
import numpy as np

A = cv2.imread("/tmp/v8635-visual/A-before-zen.png").astype(float)
B = cv2.imread("/tmp/v8635-visual/B-in-zen.png").astype(float)
C = cv2.imread("/tmp/v8635-visual/C-after-zen.png").astype(float)
