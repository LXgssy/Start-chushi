#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v8.7.21 zip 逐串新鲜度验证（Task 154 律：转义环境锚串选不跨引号/换行边界形态）"""
import zipfile, sys, json, re

ZIP = "/tmp/beta-wt/download/v8.7.21/ChuShi-NewTab-v8.7.21.zip"
...
if man.get("version") == "8.7.21":
    ok += 1
else:
    bad.append("manifest version != 8.7.21")

# ---- 2) 内容脚本逐字锚（ext-card.js 原样入包） ----
...
# ---- 5) 官方预设内嵌 widget（v8.7.20 词钮律零回归 + 版本退役） ----
...
# ---- 6) 版本串 ----
has(alljs, '"8.7.21"', "chunk: 版本串 8.7.21（changelog 条目）")
has(alljs, "音乐桥抗休眠终局", "chunk: v8.7.21 changelog 标题")

print(f"\n===== verify-zip-v8721: {ok} OK / {len(bad)} FAIL =====")
for b in bad:
    print("  FAIL:", b)
