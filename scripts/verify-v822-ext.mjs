// v8.2.2 扩展级 e2e（三态悬浮卡 + 完全体歌词 + 零跳转 + 封面态拖动 + 高光保持，真浏览器 + 真模拟器）
// 取证律：
...
//   F6 完全体歌词渲染（截图终审）
//   F7 标准态封面禁拖 + 主体把手可拖（探证）
//   F8 标准态点进度条 → 完全体且不触发 openPanel（visible 断言）
//   F9 零致命报错
//   F10 v8.2.2 封面态按住拖动移窗 + 单击展开（拖后不误触）
//   F11 v8.2.2 点歌名/主体零跳转（不开面板页、态不变）
...
const profileDir = mkdtempSync(join(tmpdir(), "ext-profile-822-"));
...
/* 动态坐标：clampPos 会随态改写 pos（full 时 x 被钆到 948）——一切按钮坐标
   ...
   v8.2.2 布局：标准卡顶部钮组独立顶带（padding-top 30px）——钮组中心
   y≈T+18；行区 y≈T+52；进度条 y≈T+88 */
function btns(pr, kind) {
  const L = pr.left, T = pr.top;
  if (kind === "mini") return {
    full: [L + 264 - 19, T + 18], coverBtn: [L + 264 - 43, T + 18],
    rail: [L + 116, T + 88], cov: [L + 34, T + 52], meta: [L + 106, T + 52],
  };
  if (kind === "full") return {
...
if (reachable) {
  ...
  /* F11 v8.2.2 零跳转：点歌名（meta）不开面板页、态不变、仍在本页 */
  {
  ...
  await p2.screenshot({ path: `${SHOTS}/v822-a-full.png` }).catch(() => {});
  ...
  await sleep(1600);
  await p2.screenshot({ path: `${SHOTS}/v822-b-full-lyric.png` }).catch(() => {});
  ...
  await p2.screenshot({ path: `${SHOTS}/v822-c-full-rail.png` }).catch(() => {});
  ...
  await p2.screenshot({ path: `${SHOTS}/v822-d-cover.png` }).catch(() => {});

  /* F10 v8.2.2 封面态按住拖动：整卡即把手——拖 ≈100px 移窗，宽度不变 */
  ...
  await p2.screenshot({ path: `${SHOTS}/v822-e-cover-dragged.png` }).catch(() => {});

  /* F5c 封面态单击 → 标准态（拖后 350ms click 闸先过期；cover 中心动态） */
  await sleep(450);
  ...
  await p2.screenshot({ path: `${SHOTS}/v822-e-dragged.png` }).catch(() => {});
