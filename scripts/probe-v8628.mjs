// v8.6.28 探针——磨砂存活 2.0（壳体入退场零 opacity）+ 阴影同拍 + 壁纸轻纱 3px：
...
const ROOT = "/tmp/ext-v868";
const ZIP = "/tmp/my-project/download/v8.6.28/ChuShi-NewTab-v8.6.28.zip";
const MOCK = "/tmp/v868-mock";
const PORT = 26997;
const SHOTS = "/tmp/probe-v868-shots";

rmSync(ROOT, { recursive: true, force: true });
rmSync(MOCK, { recursive: true, force: true });
rmSync("/tmp/ext-v868-profile", { recursive: true, force: true });
...
const browser = await withTimeout(
  chromium.launchPersistentContext("/tmp/ext-v868-profile", {
...
/* v8.6.28：抽屉形态的隐形布局克隆（.cl-layout-ghost）与真实抽屉共享全部
...
const freezeIntro = (frame, delay) => frame.evaluate(`(() => {
  ...
  /* v8.6.28：入场播完组件会摘 intro 类（根治拖拽让位重播）——按永驻语义
...
try {
  ...
  /* ---------- T3 v8.6.28 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  gate("T3b 包裹层 rise 单通道（v8.6.28 投影退役）",
  ...
  gate("T3h 冻结@12%: 霜层 blur 凝聚中（v8.6.28 底层律：见证由 opacity 改 blur 值，1<blur<14）",
  ...
  /* ---------- T4 v8.6.28 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  await page.keyboard.press("Escape");
  await sleep(700);
  await page.mouse.click(90, 620, { button: "middle" }); // 开
  await sleep(900); /* v8.6.28：650→900——远离霜层 0.24s 结束点，qCl 首命中是常驻叶
  ...
  /* v8.6.28 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  /* v8.6.28：退场见证改 transition 事件（确定性）——rAF 值采样在探针高负载下
  ...
  const exitCurve = await af.evaluate(() => new Promise((res) => {
    ...
    const tick = () => {
      rows.push({
        ...
        /* v8.6.28 补丁：扩展 iframe 环境 backdrop-filter transitionend 派发不可靠
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.28"));
    return hit ? "8.6.28" : null;
  });
  gate("T9 更新日志首条 8.6.28", logFirst === "8.6.28", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.28 核心：浅色「瓷釉」重写 ----------
  ...
  gate("TL3 v8.6.28 磁贴统一深色釉：浅色霜层=暗雾 rgba(12,12,18,0.2)",
  ...
  /* TL13c v8.6.28：dock 面板 glass-card 磨砂在线（浅色） */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  /* TL13c1 v8.6.28：面板出生窗——content-focus 在玻璃内层（后代 filter/opacity 不触
  ...
  const birth = await af.evaluate(() => new Promise((res) => {
    ...
    const poll = () => {
      ...
      if (card) {
        ...
        const inner = card.querySelector(":scope > .content-focus");
  ...
  gate("TL13c1 面板出生窗：玻璃祖先零 filter/opacity，内层 content-focus 在飞，卡 panel-fade+磨砂在线",
    !!birth && birth.wrapAnim === "none" && birth.wrapFilter === "none" && birth.wrapOp === "1" &&
    birth.innerAnim === "content-focus" && birth.innerFilter.includes("blur(") &&
  ...
  /* TL16d/e v8.6.28：互切窗行为见证——双卡同框但单玻璃（旧卡钉位 z0 停散、
     新卡 z1 停凝、壳挂 cl-switching）；落定后旧卡卸载、玻璃恒在 */
  ...
  const switchWin = await af.evaluate(() => new Promise((res) => {
    ...
    const poll = () => {
      ...
      if (stage && cards.length >= 2) {
        res({
          stageCls: stage.className,
          cards: cards.map((c) => {
            const w = c.parentElement;
            const cs = getComputedStyle(w);
            return { pos: cs.position, z: cs.zIndex, anim: getComputedStyle(c).animationName,
              op: cs.opacity, vis: cs.visibility };
          }),
        });
      } else if (performance.now() - t0 < 900) requestAnimationFrame(poll);
      else res(null);
    };
    requestAnimationFrame(poll);
  }));
  gate("TL16d 互切窗：双卡同框但单玻璃（旧卡 absolute z0 anim=none、新卡 relative z1 anim=none、壳 cl-switching）",
    !!switchWin && switchWin.stageCls.includes("cl-switching") &&
    switchWin.cards.some((c) => c.pos === "absolute" && c.z === "0" && c.anim === "none") &&
    switchWin.cards.some((c) => c.pos === "relative" && c.z === "1" && c.anim === "none"),
    switchWin ? JSON.stringify(switchWin) : "900ms 内未观测到双卡同框");
  await sleep(800);
  const settled27 = await af.evaluate(() => {
    const stage = document.querySelector(".cl-stage");
    const cards = document.querySelectorAll(".glass-card.cl-panel");
    return { cls: stage ? stage.className : "", n: cards.length,
      anim: cards.length ? getComputedStyle(cards[0]).animationName : "" };
  });
  gate("TL17c 互切窗零残留（退场卡同帧隐没：absolute z0 + opacity=0 + visibility=hidden）",
    !!switchWin && switchWin.stageCls.includes("cl-switching") &&
    switchWin.cards.some((c) => c.pos === "absolute" && c.op === "0" && c.vis === "hidden"),
    switchWin ? JSON.stringify(switchWin) : "900ms 内未观测到双卡同框");
  gate("TL16e 互切落定：旧卡卸载、新卡玻璃恒在（anim=none 底不散）、壳保持 cl-switching",
    settled27.n === 1 && settled27.anim === "none" && settled27.cls.includes("cl-switching"),
    JSON.stringify(settled27));
  ...
  /* TL13c2 v8.6.28：深色 glass-card 磨砂在线 */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  await sleep(900);
  ...
  /* ---------- TL9 v8.6.28 重写：掠影白字统一 + 零光效 + vignette 退役 + 常驻壁纸模糊 ----------
  ...
  gate("TL9c vignette 在掠影下退役（壁纸上的顶部白光带根除）",
    ...
    `vig=${pmProbe.vigBg}`);
  gate("TL9d 壁纸滤镜退役（v8.6.28 用户指令：.photo-blur 元素不存在）",
    pmProbe.blurBf === null,
    `bf=${pmProbe.blurBf}`);
  ...
  /* ---------- TL10/TL11 v8.6.28：抽屉形态壁纸不模糊 + 时钟搜索与常驻同位 ---------- */
  await af.evaluate(() => {
    const raw = JSON.parse(localStorage.getItem("start:settings") || "{}");
  ...
  /* ---------- TL14 v8.6.28：磨砂存活 2.0 + 阴影同拍（CSSOM 扫描，稳态无关时序） ---------- */
  {
    ...
    gate("TL14c 玻璃卡退场级联在位（closing 投影规则已随 v8.6.28 退役）",
      !!cssScan.cascade,
    ...
    /* ---------- TL14g/h v8.6.28 感知同步律（blur 领先/驻留） ---------- */
    ...
    /* ---------- TL15 v8.6.28：滤镜退役/分割线同拍/散场下沉/z-48 退役/双渲染 ---------- */
    ...
    gate("TL15b 互切散场通道下沉（view-exit 主规则无 animation + 玻璃卡/内容级联在位 + 部件分支保留）",
      !/animation\s*:/.test(vxMain) && !!cssScan.viewExitWidget && !!cssScan.viewExitCard && !!cssScan.viewExitContent,
      `mainAnim=${/animation\s*:/.test(vxMain)} widget=${!!cssScan.viewExitWidget} card=${!!cssScan.viewExitCard} content=${!!cssScan.viewExitContent}`);
    ...
    /* ---------- TL16 v8.6.28：互切单玻璃律（面板切换不再叠加两个面板） ---------- */
    ...
    gate("TL16b 入场卡恒浮通道（.view-top relative + z1）",
      !!cssScan.viewTopRule && /position\s*:\s*relative/.test((cssScan.viewTopRule || "").replace(/\s+/g, " ")) && /z-index\s*:\s*1/.test((cssScan.viewTopRule || "").replace(/\s+/g, " ")),
      `viewTop=${cssScan.viewTopRule ? cssScan.viewTopRule.replace(/\s+/g, " ").slice(0, 90) : "NULL"}`);
    gate("TL16c 互切整卡动画双停规则在位（.cl-switching .glass-card.panel-rise；none 行为由 TL16d/e 真实卡片采样见证）",
      !!cssScan.switchCardRule && /animation\s*:/.test((cssScan.switchCardRule || "").replace(/\s+/g, " ")),
      `rule=${cssScan.switchCardRule ? cssScan.switchCardRule.replace(/\s+/g, " ").slice(0, 100) : "NULL"}`);
    /* ---------- TL17 v8.6.28：互切内容零残留律（切换不再看见上一个面板） ---------- */
    gate("TL17a 互切退场同帧隐没规则在位（.cl-switching .view-exit：animation 通道 + opacity 0 + visibility hidden；none 行为由 TL17c 真实退场卡采样见证——CSSOM 简写序列化为长手形式，字面 none 不匹配）",
      !!cssScan.switchExitRule && /animation\s*:/.test((cssScan.switchExitRule || "").replace(/\s+/g, " ")) &&
      /opacity\s*:\s*0/.test((cssScan.switchExitRule || "").replace(/\s+/g, " ")) &&
      /visibility\s*:\s*hidden/.test((cssScan.switchExitRule || "").replace(/\s+/g, " ")),
      `rule=${cssScan.switchExitRule ? cssScan.switchExitRule.replace(/\s+/g, " ").slice(0, 140) : "NULL"}`);
    gate("TL17b 互切退场内容散场双停规则在位（.cl-switching .view-exit .content-focus：animation 通道压过 content-defocus 级联——CSSOM 简写序列化为长手形式，none 行为由 TL17c 见证）",
      !!cssScan.switchExitContent && /animation\s*:/.test((cssScan.switchExitContent || "").replace(/\s+/g, " ")),
      `rule=${cssScan.switchExitContent ? cssScan.switchExitContent.replace(/\s+/g, " ").slice(0, 140) : "NULL"}`);
  }
  ...
  console.log(`\n===== v8.6.28 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
