// v8.6.25 探针——磨砂存活 2.0（壳体入退场零 opacity）+ 阴影同拍 + 壁纸轻纱 3px：
...
const ZIP = "/tmp/my-project/download/v8.6.25/ChuShi-NewTab-v8.6.25.zip";
...
/* v8.6.25：抽屉形态的隐形布局克隆（.cl-layout-ghost）与真实抽屉共享全部
...
const freezeIntro = (frame, delay) => frame.evaluate(`(() => {
  ...
  /* v8.6.25：入场播完组件会摘 intro 类（根治拖拽让位重播）——按永驻语义
...
try {
  ...
  /* ---------- T3 v8.6.25 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  gate("T3b 包裹层 rise 单通道（v8.6.25 投影退役）",
  ...
  gate("T3h 冻结@12%: 霜层 blur 凝聚中（v8.6.25 底层律：见证由 opacity 改 blur 值，1<blur<14）",
  ...
  /* ---------- T4 v8.6.25 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  await page.keyboard.press("Escape");
  await sleep(700);
  await page.mouse.click(90, 620, { button: "middle" }); // 开
  await sleep(900); /* v8.6.25：650→900——远离霜层 0.24s 结束点，qCl 首命中是常驻叶
  ...
  /* v8.6.25 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  /* v8.6.25：退场见证改 transition 事件（确定性）——rAF 值采样在探针高负载下
  ...
  gate("T6a 稳态退场：叶淡出 + 纱罩 blur 过渡真实走完（v8.6.25 transition 事件见证）",
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.25"));
    return hit ? "8.6.25" : null;
  });
  gate("T9 更新日志首条 8.6.25", logFirst === "8.6.25", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.25 核心：浅色「瓷釉」重写 ----------
  ...
  gate("TL3 v8.6.25 磁贴统一深色釉：浅色霜层=暗雾 rgba(12,12,18,0.2)",
  ...
  /* TL13c v8.6.25：dock 面板 glass-card 磨砂在线（浅色） */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  /* TL13c1 v8.6.25：面板出生窗——content-focus 在玻璃内层（后代 filter/opacity 不触
  ...
  /* TL13c2 v8.6.25：深色 glass-card 磨砂在线 */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  await sleep(900);
  ...
  /* ---------- TL9 v8.6.25 重写：掠影白字统一 + 零光效 + vignette 退役 + 常驻壁纸模糊 ----------
  ...
  gate("TL9c vignette 在掠影下退役（壁纸上的顶部白光带根除）",
    ...
    `vig=${pmProbe.vigBg}`);
  gate("TL9d 常驻+掠影：壁纸极轻薄纱（v8.6.25 再减 blur 3px）",
  ...
  /* ---------- TL10/TL11 v8.6.25：抽屉形态壁纸不模糊 + 时钟搜索与常驻同位 ---------- */
  await af.evaluate(() => {
    const raw = JSON.parse(localStorage.getItem("start:settings") || "{}");
  ...
  /* ---------- TL14 v8.6.25：磨砂存活 2.0 + 阴影同拍（CSSOM 扫描，稳态无关时序） ---------- */
  {
    ...
    gate("TL14c 玻璃卡退场级联在位（closing 投影规则已随 v8.6.25 退役）",
      !!cssScan.cascade,
    ...
    /* ---------- TL14g/h v8.6.25 感知同步律（blur 领先/驻留） ---------- */
  ...
  console.log(`\n===== v8.6.25 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
