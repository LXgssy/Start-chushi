// v8.7.5 视觉目检（Task 146 ㊼）：抽屉纱罩关键帧分段散场帧序（无残留无突变）
// B 组：抽屉关闭分段散场帧序（中键关 → 连拍：0-40% 快启缓落 28→5px、40-100% 匀速 5→1px、
//       hidden @0.30s——尾段匀速无爬行、终帧 sat 归位零色感突变）
import { chromium } from "playwright-core";
import crypto from "crypto";

const ROOT = "/tmp/ext-beta";
const SHOTS = "/tmp/v875-visual";
...
const browser = null;
const ctx = await chromium.launchPersistentContext("/tmp/v875-profile", {
