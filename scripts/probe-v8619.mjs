// v8.6.19 探针——快捷服务入场三通道 + 打断并行动画 + v8.6.6 启动崩溃修复：
...
const ZIP = "/tmp/my-project/download/v8.6.19/ChuShi-NewTab-v8.6.19.zip";
...
/* v8.6.19：抽屉形态的隐形布局克隆（.cl-layout-ghost）与真实抽屉共享全部
...
try {
  ...
  /* ---------- T3 v8.6.19 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  /* ---------- T4 v8.6.19 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  ...
  /* v8.6.19 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.19"));
    return hit ? "8.6.19" : null;
  });
  gate("T9 更新日志首条 8.6.19", logFirst === "8.6.19", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.19 核心：浅色「瓷釉」重写 ----------
  ...
  /* ---------- TL9 v8.6.19 重写：掠影白字统一 + 零光效 + vignette 退役 + 常驻壁纸模糊 ----------
  ...
  gate("TL9c vignette 在掠影下退役（壁纸上的顶部白光带根除）",
    ...
    `vig=${pmProbe.vigBg}`);
  gate("TL9d 常驻+掠影：壁纸高斯模糊在线（blur 24px）",
    /blur\(24px\)/.test(pmProbe.blurBf || ""),
    `bf=${pmProbe.blurBf}`);
  await page.screenshot({ path: SHOTS + "/06-wp-light.png" });

  /* ---------- TL10/TL11 v8.6.19：抽屉形态壁纸不模糊 + 时钟搜索与常驻同位 ---------- */
  await af.evaluate(() => {
    const raw = JSON.parse(localStorage.getItem("start:settings") || "{}");
  ...
  console.log(`\n===== v8.6.19 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
