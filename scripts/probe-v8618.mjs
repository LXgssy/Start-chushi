// v8.6.18 探针——快捷服务入场三通道 + 打断并行动画 + v8.6.6 启动崩溃修复：
...
const ZIP = "/tmp/my-project/download/v8.6.18/ChuShi-NewTab-v8.6.18.zip";
...
try {
  ...
  /* ---------- T3 v8.6.18 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  /* ---------- T4 v8.6.18 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  ...
  /* v8.6.18 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.18"));
    return hit ? "8.6.18" : null;
  });
  gate("T9 更新日志首条 8.6.18", logFirst === "8.6.18", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.18 核心：浅色「瓷釉」重写 ----------
  ...
  console.log(`\n===== v8.6.18 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
