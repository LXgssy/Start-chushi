// v8.6.34 探针——互切拉伸律：面板切换动画全面重写（删 v8.6.25-28 补丁链，恢复最初拉伸动效）：
...
const ZIP = "/tmp/my-project/download/v8.6.34/ChuShi-NewTab-v8.6.34.zip";
...
/* v8.6.34：抽屉形态的隐形布局克隆（.cl-layout-ghost）与真实抽屉共享全部
...
const freezeIntro = (frame, delay) => frame.evaluate(`(() => {
  ...
  /* v8.6.34：入场播完组件会摘 intro 类（根治拖拽让位重播）——按永驻语义
...
try {
  ...
  /* ---------- T3 v8.6.34 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  gate("T3b 包裹层 rise 单通道（v8.6.34 投影退役）",
  ...
  gate("T3h 冻结@12%: 霜层 blur 凝聚中（v8.6.34 底层律：见证由 opacity 改 blur 值，1<blur<14）",
  ...
  /* ---------- T4 v8.6.34 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  await page.keyboard.press("Escape");
  await sleep(700);
  await page.mouse.click(90, 620, { button: "middle" }); // 开
  await sleep(900); /* v8.6.34：650→900——远离霜层 0.24s 结束点，qCl 首命中是常驻叶
  ...
  /* v8.6.34 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  /* v8.6.34：退场见证改 transition 事件（确定性）——rAF 值采样在探针高负载下
  ...
  const exitCurve = await af.evaluate(() => new Promise((res) => {
    ...
    const tick = () => {
      rows.push({
        ...
        /* v8.6.34 补丁：扩展 iframe 环境 backdrop-filter transitionend 派发不可靠
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.34"));
    return hit ? "8.6.34" : null;
  });
  gate("T9 更新日志首条 8.6.34", logFirst === "8.6.34", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.34 核心：浅色「瓷釉」重写 ----------
  ...
  gate("TL3 v8.6.34 磁贴统一深色釉：浅色霜层=暗雾 rgba(12,12,18,0.2)",
  ...
  /* TL13c v8.6.34：dock 面板 glass-card 磨砂在线（浅色） */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  /* TL13c1 v8.6.34：面板出生窗——content-focus 在玻璃内层（后代 filter/opacity 不触
  ...
  const birth = await af.evaluate(() => new Promise((res) => {
    ...
    const poll = () => {
      ...
      if (card) {
        /* v8.6.34：内容层模糊聚拢在飞——blur 只存在于 0.3s 动画窗内，出生即采样
  ...
  /* TL16d/e+TL17c v8.6.34：互切拉伸律行为见证——单卡律/同卡律/零残留律 */
  ...
  /* TL16f v8.6.34 互切底锚律行为见证：壳体底缘恒定 + 玻璃卡底永不悬空 + 底锚锁在位。
    ...
    requestAnimationFrame(() => {
      ...
      const poll = () => {
        ...
        if (st && card) {
          ...
          /* v8.6.34 玻璃壳满窗律：卡高每帧==壳高（h-full 贴随弹簧，底边恒定） */
  ...
  gate("TL16f 互切底锚律 v8.6.34 玻璃壳满窗：切换窗 900ms 壳体底缘恒定（≤2px）+ 玻璃卡底永不悬空（gap≤1px）+ 卡高贴随壳高（h-full 每帧同高）",
  ...
  /* TL18a v8.6.34 雾化 filter 回归 + 玻璃载体零毒退场（静态门）：雾化 blur(12px)
  ...
  /* v8.6.34 反转：三载体禅退场=时钟同款模糊雾化（opacity:0 + blur(12px)），
  ...
  gate("TL18b' v8.6.34 模糊雾化退场：zen-gone/zen-dock/search-pill 三条 html.zen 规则 opacity:0+blur(12px)+零 scale+visibility 末帧隐没 + .zen-gone 基线 opacity/filter 过渡在位",
  ...
  /* TL20 v8.6.34 退禅磨砂复原双保险（源码门）：defrostGlass 在位（display 往返 +
  ...
  /* TL18b v8.6.34 磨砂底层化·图标材质：.tile-frost 材质基线在位（样式表常驻声明）+
  ...
  /* TL19 v8.6.34 禅窗零毒行为门：docked 磁贴墙在禅进出全程 opacity=1/filter=none，
  ...
  const zenPhase = await af.evaluate(async () => {
    ...
    /* v8.6.34 真实路径：进/退禅一律 dblclick 派发（React onDblClick →
  ...
  gate("TL19a v8.6.34 禅窗模糊雾化：intro 排空 + dblclick 进禅 + 磁贴墙 opacity=0+blur(12px)+visibility=hidden + 雾化 blur(12px) 在时钟段在线",
  ...
  gate("TL19b v8.6.34 退禅复原（defrostGlass 双保险）：visibility=visible + opacity=1 + filter=none + .tile-frost 磨砂 blur(14px) saturate(1.6) 恒在线",
  ...
  /* TL13c2 v8.6.34：深色 glass-card 磨砂在线 */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  await sleep(900);
  ...
  /* ---------- TL9 v8.6.34 重写：掠影白字统一 + 零光效 + vignette 退役 + 常驻壁纸模糊 ----------
  ...
  gate("TL9c vignette 在掠影下退役（壁纸上的顶部白光带根除）",
    ...
    `vig=${pmProbe.vigBg}`);
  gate("TL9d 壁纸滤镜退役（v8.6.34 用户指令：.photo-blur 元素不存在）",
    pmProbe.blurBf === null,
    `bf=${pmProbe.blurBf}`);
  ...
  /* ---------- TL10/TL11 v8.6.34：抽屉形态壁纸不模糊 + 时钟搜索与常驻同位 ---------- */
  await af.evaluate(() => {
    const raw = JSON.parse(localStorage.getItem("start:settings") || "{}");
  ...
  /* ---------- TL14 v8.6.34：磨砂存活 2.0 + 阴影同拍（CSSOM 扫描，稳态无关时序） ---------- */
  {
    ...
    gate("TL14c 玻璃卡退场级联在位（closing 投影规则已随 v8.6.34 退役）",
      !!cssScan.cascade,
    ...
    /* ---------- TL14g/h v8.6.34 感知同步律（blur 领先/驻留） ---------- */
    ...
    /* ---------- TL15 v8.6.34：滤镜退役/分割线同拍/散场下沉/z-48 退役/双渲染 ---------- */
    ...
    /* ---------- TL16 v8.6.34：互切单玻璃律（面板切换不再叠加两个面板） ---------- */
    ...
    /* ---------- TL17 v8.6.34：互切内容零残留律（切换不再看见上一个面板） ---------- */
    gate("TL17a 互切内容层过场在位（.cl-panel-content 模糊聚拢淡入 + .panel-sink 级联淡出——v8.6.34 模糊聚拢回归）",
  ...
  console.log(`\n===== v8.6.34 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
