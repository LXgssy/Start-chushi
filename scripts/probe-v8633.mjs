// v8.6.33 探针——互切拉伸律：面板切换动画全面重写（删 v8.6.25-28 补丁链，恢复最初拉伸动效）：
...
const ZIP = "/tmp/my-project/download/v8.6.33/ChuShi-NewTab-v8.6.33.zip";
...
/* v8.6.33：抽屉形态的隐形布局克隆（.cl-layout-ghost）与真实抽屉共享全部
...
const freezeIntro = (frame, delay) => frame.evaluate(`(() => {
  ...
  /* v8.6.33：入场播完组件会摘 intro 类（根治拖拽让位重播）——按永驻语义
...
try {
  ...
  /* ---------- T3 v8.6.33 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  gate("T3b 包裹层 rise 单通道（v8.6.33 投影退役）",
  ...
  gate("T3h 冻结@12%: 霜层 blur 凝聚中（v8.6.33 底层律：见证由 opacity 改 blur 值，1<blur<14）",
  ...
  /* ---------- T4 v8.6.33 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  await page.keyboard.press("Escape");
  await sleep(700);
  await page.mouse.click(90, 620, { button: "middle" }); // 开
  await sleep(900); /* v8.6.33：650→900——远离霜层 0.24s 结束点，qCl 首命中是常驻叶
  ...
  /* v8.6.33 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  /* v8.6.33：退场见证改 transition 事件（确定性）——rAF 值采样在探针高负载下
  ...
  const exitCurve = await af.evaluate(() => new Promise((res) => {
    ...
    const tick = () => {
      rows.push({
        ...
        /* v8.6.33 补丁：扩展 iframe 环境 backdrop-filter transitionend 派发不可靠
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.33"));
    return hit ? "8.6.33" : null;
  });
  gate("T9 更新日志首条 8.6.33", logFirst === "8.6.33", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.33 核心：浅色「瓷釉」重写 ----------
  ...
  gate("TL3 v8.6.33 磁贴统一深色釉：浅色霜层=暗雾 rgba(12,12,18,0.2)",
  ...
  /* TL13c v8.6.33：dock 面板 glass-card 磨砂在线（浅色） */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  /* TL13c1 v8.6.33：面板出生窗——content-focus 在玻璃内层（后代 filter/opacity 不触
  ...
  const birth = await af.evaluate(() => new Promise((res) => {
    ...
    const poll = () => {
      ...
      if (card) {
        /* v8.6.33：内容层模糊聚拢在飞——blur 只存在于 0.3s 动画窗内，出生即采样
  ...
  /* TL16d/e+TL17c v8.6.33：互切拉伸律行为见证——单卡律/同卡律/零残留律 */
  ...
  /* TL16f v8.6.33 互切底锚律行为见证：壳体底缘恒定 + 玻璃卡底永不悬空 + 底锚锁在位。
    ...
    requestAnimationFrame(() => {
      ...
      const poll = () => {
        ...
        if (st && card) {
          ...
          /* v8.6.33 玻璃壳满窗律：卡高每帧==壳高（h-full 贴随弹簧，底边恒定） */
  ...
  gate("TL16f 互切底锚律 v8.6.33 玻璃壳满窗：切换窗 900ms 壳体底缘恒定（≤2px）+ 玻璃卡底永不悬空（gap≤1px）+ 卡高贴随壳高（h-full 每帧同高）",
  ...
  /* TL18a v8.6.33 雾化 filter 回归 + 玻璃载体零毒退场（静态门）：雾化 blur(12px)
  ...
  gate("TL18b' 玻璃载体零毒退场：zen-gone/zen-dock/search-pill 三条 html.zen 规则 visibility+scale(0) 且零 opacity/零 filter + .zen-gone 基线在位",
    noPoison(zenScan.gone) && noPoison(zenScan.dock) && noPoison(zenScan.pill) && !!zenScan.goneBase,
  ...
  /* TL18b v8.6.33 磨砂底层化·图标材质：.tile-frost 材质基线在位（样式表常驻声明）+
  ...
  /* TL19 v8.6.33 禅窗零毒行为门：docked 磁贴墙在禅进出全程 opacity=1/filter=none，
  ...
  af = await page.reload({ waitUntil: "load" }).then(() => bootNewTab(page));
  const dockedBooted = await af.evaluate(() => !!document.querySelector("section.zen-gone"));
  ...
  const zenPhase = await af.evaluate(async () => {
    ...
    const inZen = snap();
    html.classList.remove("zen");
    t0 = performance.now();
    while (performance.now() - t0 < 1000) await nextFrame();
    const afterZen = snap();
    return { drained, inZen, afterZen };
  });
  gate("TL19a 禅窗磁贴墙零毒：intro 动画排空 + visibility=hidden 且 opacity=1/filter=none + 雾化 blur(12px) 在时钟段在线",
    zenPhase.drained && zenPhase.inZen.secVis === "hidden" && zenPhase.inZen.secOpa === "1" && (zenPhase.inZen.secFil === "none" || zenPhase.inZen.secFil === "") && /blur\(12px\)/.test(zenPhase.inZen.clockFil || ""),
    JSON.stringify(zenPhase.inZen));
  gate("TL19b 退禅复原：visibility=visible + opacity=1 + .tile-frost 磨砂 blur(14px) saturate(1.6) 恒在线",
    zenPhase.afterZen.secVis === "visible" && zenPhase.afterZen.secOpa === "1" && /blur\(14px\)/.test(zenPhase.afterZen.frostBf || "") && /saturate\(1\.6\)/.test(zenPhase.afterZen.frostBf || ""),
  ...
  /* TL13c2 v8.6.33：深色 glass-card 磨砂在线 */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  await sleep(900);
  ...
  /* ---------- TL9 v8.6.33 重写：掠影白字统一 + 零光效 + vignette 退役 + 常驻壁纸模糊 ----------
  ...
  gate("TL9c vignette 在掠影下退役（壁纸上的顶部白光带根除）",
    ...
    `vig=${pmProbe.vigBg}`);
  gate("TL9d 壁纸滤镜退役（v8.6.33 用户指令：.photo-blur 元素不存在）",
    pmProbe.blurBf === null,
    `bf=${pmProbe.blurBf}`);
  ...
  /* ---------- TL10/TL11 v8.6.33：抽屉形态壁纸不模糊 + 时钟搜索与常驻同位 ---------- */
  await af.evaluate(() => {
    const raw = JSON.parse(localStorage.getItem("start:settings") || "{}");
  ...
  /* ---------- TL14 v8.6.33：磨砂存活 2.0 + 阴影同拍（CSSOM 扫描，稳态无关时序） ---------- */
  {
    ...
    gate("TL14c 玻璃卡退场级联在位（closing 投影规则已随 v8.6.33 退役）",
      !!cssScan.cascade,
    ...
    /* ---------- TL14g/h v8.6.33 感知同步律（blur 领先/驻留） ---------- */
    ...
    /* ---------- TL15 v8.6.33：滤镜退役/分割线同拍/散场下沉/z-48 退役/双渲染 ---------- */
    ...
    /* ---------- TL16 v8.6.33：互切单玻璃律（面板切换不再叠加两个面板） ---------- */
    ...
    /* ---------- TL17 v8.6.33：互切内容零残留律（切换不再看见上一个面板） ---------- */
    gate("TL17a 互切内容层过场在位（.cl-panel-content 模糊聚拢淡入 + .panel-sink 级联淡出——v8.6.33 模糊聚拢回归）",
  ...
  console.log(`\n===== v8.6.33 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
