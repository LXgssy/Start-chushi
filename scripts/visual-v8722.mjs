...
const SHOTS = "/tmp/v8722-visual";
const PROFILE = "/tmp/v8722-profile";
...
const onState = await wf.evaluate(() => {
  ...
  return {
    ...
    ring: getComputedStyle(bd.querySelector("circle")).stroke,
    /* 角标锚点：徽标右下角 vs 按钮右下角（4.5px 入框设计 → 差 ≈4.5px，全徽标落在方框内） */
...
judge("F6b on 态角标显形（block 9×9 accent 圆）", onState.badge === "block" && Math.abs(onState.bw - 9) <= 1 && Math.abs(onState.bh - 9) <= 1 && onState.fill === onState.acc, `display=${onState.badge} ${onState.bw}x${onState.bh} fill=${onState.fill}`);
judge("F6c 角标入框=框内词字右下角（设计 right/bottom:4.5px → 徽标整体收进方框，dR/dB=4.5±1.5）", Math.abs(onState.dR - 4.5) <= 1.5 && Math.abs(onState.dB - 4.5) <= 1.5, `dR=${onState.dR} dB=${onState.dB}`);
judge("F6d 角标白描边环在体（与点亮 accent 字形分离）", onState.ring === "rgb(255, 255, 255)", `ring=${onState.ring}`);
...
console.log(`\n===== visual-v8722: ${passCount} PASS / ${failCount} FAIL =====`);
