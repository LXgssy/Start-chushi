...
/* verify-v220.mjs — v2.2.0：插件真值绝对锚定 / 暂停恢复零累积漂移 / seek 诚实验证
 *
...
/* ---------- mock 桥状态 ---------- */
const DUR = 269.3; // 秒
let mockState = { ok: true, name: "chushi-smtc-bridge", version: "1.6.0-mock", track: null, ne: null };
...
const mock = createServer((req, res) => {
  ...
  if (u.pathname === "/api/ping") {
    ...
    res.end(JSON.stringify({ ok: true, name: "chushi-smtc-bridge", version: "1.6.0-mock" }));
    return;
  }
...
/* ---------- FT 页脚诊断 ---------- */
{
  ...
  check("FT1 页脚可见插件版本 v1.2.0", ap.includes("插件 v1.2.0"), ap);
}
/* ---------- S1 seek 生效：真值跟上 → 不弹回 ---------- */
{
  seekBehavior = "ok";
