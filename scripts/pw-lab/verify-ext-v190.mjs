...
/* verify-ext-v190.mjs — v1.9.0 扩展冒烟：
 * 加载扩展 → 渲染 → host_permissions 含 127.0.0.1:20754（v1.9.0 关键修复）→
 * 导入 .cshz → dock 音乐按钮 → 弹出面板（预热+播放态）→ 歌词/封面/进度 → 控制上行 → pageerror=0
...
/* 解包扩展 */
const extDir = mkdtempSync(tmpdir() + "/ext-v190-");
execSync(`unzip -qo /home/z/my-project/download/v1.9.0/ChuShi-NewTab-v1.9.0.zip -d ${extDir}`);
/* manifest 断言：host_permissions 必须已含本机桥 */
const mf = JSON.parse(readFileSync(extDir + "/manifest.json", "utf8"));
console.log(mf.version === "1.9.0" && mf.host_permissions.includes("http://127.0.0.1:20754/*")
  ? "✓ M0 manifest v1.9.0 + host_permissions 含 127.0.0.1:20754"
  : "✗ M0 manifest 缺陷：" + JSON.stringify(mf.host_permissions));
rmSync("/tmp/ext-v190-profile", { recursive: true, force: true });

const browser = await chromium.launchPersistentContext("/tmp/ext-v190-profile", {
  headless: true,
  channel: "chromium",
...
check("X4 弹出容器常驻预热", (await page0.locator(".cl-dockwidget").count()) === 1);
await page0.keyboard.press("Escape");
await page0.waitForTimeout(400);

/* 播放态（SMTC 时间轴缺失 + ne 兜底 + 歌词） */
mockLyric = {
...
await page0.waitForTimeout(3200);
...
check("X5 面板高度 ≈372（歌词态直开）", Math.abs(await page0.locator(".cl-dockwidget").evaluate((el) => el.getBoundingClientRect().height) - 372) < 12,
  `h=${await page0.locator(".cl-dockwidget").evaluate((el) => el.getBoundingClientRect().height)}`);
check("X6 歌名", (await wf.locator("#t1f").textContent()) === "晴天");
check("X7 进度条 ≈21%（插件时钟兜底）", (() => { return true; })() && (await (async () => {
  const w = await wf.locator("#barF").evaluate((el) => parseFloat(el.style.width) || 0);
  return w > 19 && w < 24;
})()));
check("X8 歌词行高亮", (await wf.locator("#ly .lyline.on").count()) === 1);
check("X9 逐字扫色 act", (await wf.locator("#ly .lyw.act").count()) >= 1);
check("X10 SMTC 封面 dataURL", ((await wf.locator("#aF").getAttribute("src")) || "").startsWith("data:image/png"));
/* 控制上行（DOM click：嵌套 srcdoc 鼠标事件丢失先例） */
const b0 = controlLog.length;
await wf.locator("#pF0").evaluate((el) => el.click());
...
check("X12 收起键关闭面板", (await page0.locator(".cl-dockwidget").evaluate((el) => el.getBoundingClientRect().height)) < 2);
check("X13 pageerror = 0", errors.length === 0, errors.join(" | "));

await browser.close();
mock.close();
console.log(results.every(Boolean) ? `\n扩展冒烟 ${results.length + 1} 项全过 ✓` : `\n${results.filter((x) => !x).length} 项失败 ✗`);
