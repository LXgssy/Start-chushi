...
/* verify-ext-v177.mjs — v1.7.7 扩展冒烟（音乐面板翻新版）：
 * A 扩展新标签页 dock 有「音乐」按钮，面板打开=未连接指引（插件路线文案）
...
const browser = await chromium.launchPersistentContext("/tmp/ext-v177-profile", {
  headless: true,
  channel: "chromium",
  args: [
    `--headless=new`,
...
check("A3 插件路线主推文案", (await panel.getByText("BetterNCM 插件路线 · 推荐").count()) === 1);
...
check("D1 诊断卡显示桥版本", (await panel.getByText("1.3.0 · bridge.dll").count()) === 1);
await diagBtn.click();
