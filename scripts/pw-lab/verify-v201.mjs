...
/* verify-v201.mjs — v2.0.1：暂停归零/恢复重头/seek 拽回/按钮迟钝/闪白/逐字闪动 六联根治
...
/* ---------- O 播放/暂停乐观翻转 ---------- */
{
  ...
  const yHidden0 = await wFrame().locator("#yI").evaluate((el) => el.classList.contains("hd"));
  check("O0 点击前显示暂停图标（播放中）", yHidden0 === true);
  controlLog.length = 0;
  await wFrame().locator("#pB").click();
  await page.waitForTimeout(200); // 不等任何快照/轮询
  const yHidden1 = await wFrame().locator("#yI").evaluate((el) => el.classList.contains("hd"));
  const nVis1 = await wFrame().locator("#nI").evaluate((el) => !el.classList.contains("hd"));
  ...
  const yVis2 = await wFrame().locator("#yI").evaluate((el) => !el.classList.contains("hd"));
  const nVis2 = await wFrame().locator("#nI").evaluate((el) => !el.classList.contains("hd"));
  check("O3 真实暂停态确认（播放三角保持、暂停条不回）", yVis2 === true && nVis2 === false,
    `yI.vis=${yVis2} nI.vis=${nVis2}`);
  // mock 回 playing=true → 快照广播 → 翻回播放中
  mockState = { ...mockState, track: { ...mockState.track, playing: true } };
  await page.waitForTimeout(1600);
  const yHidden3 = await wFrame().locator("#yI").evaluate((el) => el.classList.contains("hd"));
  const nVis3 = await wFrame().locator("#nI").evaluate((el) => !el.classList.contains("hd"));
