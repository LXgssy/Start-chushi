...
/* verify-ext-v200.mjs — v2.0.0 扩展冒烟：
 * 加载扩展 → 渲染 → manifest v2.0.0 + host_permissions（127.0.0.1:20754）→
...
execSync(`unzip -qo /home/z/my-project/download/v2.0.0/ChuShi-NewTab-v2.0.0.zip -d ${extDir}`);
const mf = JSON.parse(readFileSync(extDir + "/manifest.json", "utf8"));
const m0ok = mf.version === "2.0.0" && mf.host_permissions.includes("http://127.0.0.1:20754/*");
console.log(m0ok ? "✓ M0 manifest v2.0.0 + host_permissions 含 127.0.0.1:20754" : "✗ M0 manifest 缺陷：" + JSON.stringify(mf.host_permissions));
