// v1.5.0 扩展冒烟：--load-extension 真浏览器加载 → 新标签页渲染 → 导入液态玻璃 → WebGL 引擎生效
import { chromium } from "playwright-core";
import fs from "node:fs";
import { createHash } from "node:crypto";
import { execSync } from "node:child_process";

const STAGE = "/tmp/ext-smoke-v15";
fs.rmSync(STAGE, { recursive: true, force: true });
fs.mkdirSync(STAGE, { recursive: true });
execSync(`cd ${STAGE} && unzip -q /home/z/my-project/download/v1.5.0/ChuShi-NewTab-v1.5.0.zip`);

const userData = "/tmp/ext-profile-v15";
...
await page.screenshot({ path: "/home/z/my-project/scripts/pw-lab/shots/ext-v15.png" });
await browser.close();
console.log("✓ v1.5.0 扩展冒烟通过");
