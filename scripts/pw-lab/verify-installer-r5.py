#!/usr/bin/env python3
# verify-installer-r5.py — 初始音乐桥 r5 全量回归验证
# 覆盖：PS1 词法平衡 / Clean-Path 孪生模拟 / bat 调用行 / zip 布局 / 关键修复序 /
#       C 源静态特征 / bridge-js 一致性 / exe 版本字符串 / r5 diag 读出与 PNA
...
# ---------- 4b. 使用说明版本与 r5 段 ----------
print("[4b] 使用说明")
ud = z.read("ChuShiBridge-Setup/使用说明.md").decode("utf-8")
check("说明标题 v2.0.4", ud.startswith("# 初始音乐桥 · 独立版（ChuShiBridge Standalone）v2.0.4"))
check("说明含 r5 修复段", "v2.0.4 相对 2.0.3 的修复" in ud and "snapOk" in ud)
check("说明与源一致", ud == open(f"{SRC}/installer/使用说明.md", encoding="utf-8").read())
...
check("CB_VERSION 2.0.4", '#define CB_VERSION     "2.0.4"' in hdr)
check("/api/debug 输出 attach/attachDetail", '"attach\\"' in srv_c and '"attachDetail' in srv_c)
check("debug body 扩容 2048", "char body[2048]" in srv_c)
...
# ---------- 7b. r5 diag 真实读出 + PNA ----------
print("[7b] r5 diag 读出 + PNA")
check("r5 共享状态 diag_json/snap_ok_flag/snap_err", all(
    k in hdr for k in ["diag_json", "snap_ok_flag", "snap_err"]))
check("r5 cb_diag_set 实现与声明", "void cb_diag_set" in srv_c and "void cb_diag_set" in hdr)
check("r5 state_free 释放 diag_json",
      srv_c.find("free(s->diag_json)") > 0 and srv_c.find("free(s->diag_json)") < srv_c.find("cmd_head") )
check("r5 record_snap_receipt 存在", "static void record_snap_receipt" in main_c)
check("r5 回执落库两处调用(附加+轮询)", main_c.count("record_snap_receipt(snap)") == 2)
check("r5 附加路径调用在 cb_attach_set(ok) 之前",
      0 < main_c.find("record_snap_receipt(snap)") < main_c.find('cb_attach_set("ok"'))
check("r5 debug 内嵌真实 diag(\"diag\":%s)", '\\"diag\\":%s' in srv_c or '"diag\\":%s' in srv_c)
check("r5 debug 新增 snapOk/snapErr", '"snapOk' in srv_c and '"snapErr' in srv_c)
check("r5 diag 兑底段(全 false)", '\\"store\\":false,\\"cmder\\":false' in srv_c)
check("r5 diag 段 ASCII 安全化嵌入", srv_c.find("json_sanitize_ascii(diagbuf") > 0)
check("r5 PNA 预检头", "Access-Control-Allow-Private-Network: true" in srv_c)

def js_src_of():
    return open(f"{SRC}/bridge-js/bridge-core.js", encoding="utf-8").read()

check("r5 JS diag 含 href", "location.href" in js_src_of() and "slice(0, 80)" in js_src_of())
...
check("JS 版本 2.0.4", 'VERSION = "2.0.4"' in js and '__v === "2.0.4"' in js)
check("JS 无 2.0.3 版本残留", 'VERSION = "2.0.3"' not in js and '__v === "2.0.3"' not in js)
...
check("exe 含 2.0.4", b"2.0.4" in exe)
check("exe 无 2.0.3/2.0.1/2.0.0 版本残留", b"2.0.3" not in exe and b"\"2.0.1\"" not in exe and b"\"2.0.0\"" not in exe)
