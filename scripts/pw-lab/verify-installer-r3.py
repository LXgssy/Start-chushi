...
check("probe 三路判据(orpheus)", '"orpheus:"' in main_c or 'orpheus:' in main_c)
check("probe webpackChunk 判据(wc)", 'wc:(function' in main_c and 'webpackChunk' in main_c and '===0)return true' in main_c)
check("probe miss 细分", "PROBE_MISS" in main_c and "PROBE_EVAL_FAIL" in main_c)
check("attach 细分日志节流(5min)", "300000" in main_c and "attach_fail_log" in main_c)
check("attach 状态全集", all(s in main_c for s in ['"ws-fail"', '"probe-eval-fail"', '"probe-miss"', '"install-fail"', '"snap-fail"', '"poll-fail"']))
...
check("ws_eval 用 ws_read_message", "ws_read_message(s, &m" in cdp_c)
check("eval 失败详情捕获", "eval-timeout" in cdp_c and "cdp-error" in cdp_c and "eval-no-value" in cdp_c)
...
check("CB_VERSION 2.0.2", '#define CB_VERSION     "2.0.2"' in hdr)
...
check("JS 版本 2.0.2", 'VERSION = "2.0.2"' in js and '__v === "2.0.2"' in js)
...
check("exe 含 2.0.2", b"2.0.2" in exe)
