...
/* verify-v175.mjs — v1.7.5 音乐面板（网易云音乐接入）端到端验证
 * A Dock「音乐」按钮 → 面板打开，未连接指引可见
...
check("A4 地址输入默认 10754", (await addr.inputValue()).includes("10754"));
...
check("C3 进度条在位", (await range.count()) === 1);
const rangeVal = await range.inputValue();
check("C4 进度值=快照位置（~30000ms）", Math.abs(Number(rangeVal) - 30000) < 2500, rangeVal);
const vol = panel.locator("input[aria-label='音量']");
check("C5 音量条渲染", (await vol.count()) === 1, String(await vol.count()));
check("C6 状态脚注已连接", (await panel.getByText("已连接 · 网易云音乐").count()) === 1);
...
/* 暂停后 UI 状态切换（mock 不响应命令，但 toggle 命令乐观发出；快照仍 playing=true） */
/* 直接改 mock 状态验证 UI 回路 */
