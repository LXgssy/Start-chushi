// v1.4.0 扩展冒烟：--load-extension 真浏览器加载 → 新标签页渲染 → 导入液态玻璃 → 内建引擎生效
import { chromium } from "playwright-core";
import fs from "node:fs";
import { createHash } from "node:crypto";
import { execSync } from "node:child_process";

const STAGE = "/tmp/ext-smoke-v14";
fs.rmSync(STAGE, { recursive: true, force: true });
fs.mkdirSync(STAGE, { recursive: true });
execSync(`cd ${STAGE} && unzip -q /home/z/my-project/download/v1.4.0/ChuShi-NewTab-v1.4.0.zip`);

const userData = "/tmp/ext-profile-v14";
...
const engine = await page.evaluate(() => {
  ...
  return {
    ...
    scaleNeg: (() => { const d = [...document.querySelectorAll("#chushi-lg-root feDisplacementMap")][0]; return d ? parseFloat(d.getAttribute("scale")) : 0; })(),
...
const pass =
  ...
  engine.marked >= 2 &&
  errors.length === 0;
console.log(pass ? "✓ 扩展冒烟通过（内建引擎实时渲染 + 负量内采样）" : `✗ 冒烟失败 errors=${JSON.stringify(errors)}`);
