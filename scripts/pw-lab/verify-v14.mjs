// v1.4.0 全链路验证 —— 液态玻璃引擎内建宿主（实时渲染 + 负量内采样 + 覆盖范围 + 图标/主题 API 共存）
...
console.log("  几何: border=" + st.pillBorder, "radius=" + st.pillRadius);
console.log("  滤镜数:", st.filterCount, "scale:", st.scale);
...
/* 物理方向判定：feDisplacementMap scale 必须为负（Apple/Kyant 负量内采样） */
const sc = parseFloat(st.scale);
if (!(sc < 0)) fail("位移方向不是负量内采样 scale=" + st.scale);
else ok("负量内采样生效（scale=" + st.scale + "，凸透镜放大，Apple 同向）");
const noExpand = await page.evaluate(() => {
  const pill = document.querySelector(".search-pill[data-lg]");
  const cs = pill ? getComputedStyle(pill) : null;
  return { border: cs ? cs.borderTopWidth : "", radiusVar: pill?.style.getPropertyValue("--lg-radius") || "" };
});
if (parseFloat(noExpand.border) >= 4) fail("引擎疑似仍在改动元素几何 border=" + noExpand.border);
else ok("引擎零几何侵入（border " + noExpand.border + " = 元素原生值，布局无扰）");
await page.screenshot({ path: `${OUT}/v14-1-engine.png` });
...
await page.screenshot({ path: `${OUT}/v14-2-realtime.png` });
...
await page.screenshot({ path: `${OUT}/v14-3-chip-core.png` });
await page.locator('nav[aria-label="快捷操作"] button[aria-label="设置"]').click();
await page.waitForTimeout(900);
