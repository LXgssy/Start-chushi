// v8.7.24 视觉端到端：播放器六项迭代（v8.7.23 全量回归门改造 + 新增互通门）
...
const ROOT = "/tmp/ext-beta-n24";
const ZIP = "/tmp/beta-wt/download/v8.7.24/ChuShi-NewTab-v8.7.24.zip";
const SHOTS = "/tmp/v8724-visual";
const PROFILE = "/tmp/v8724-profile";
...
const hub = http.createServer((req, res) => {
  ...
  const url = req.url || "";
  if (url.startsWith("/api/ping")) res.end(JSON.stringify({ ok: true, name: "chushi-music-hub", version: "mock-v8724" }));
...
judge("N6b YRC 逐字 3 行 3 词 + .ov 双层", n6.lines === 3 && n6.words === 3 && n6.ovs === 3, `lines=${n6.lines} words=${n6.words} ovs=${n6.ovs}`);
