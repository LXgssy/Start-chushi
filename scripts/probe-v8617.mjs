// v8.6.17 探针——快捷服务入场三通道 + 打断并行动画 + v8.6.6 启动崩溃修复：
...
const ZIP = "/tmp/my-project/download/v8.6.17/ChuShi-NewTab-v8.6.17.zip";
...
try {
  ...
  /* ---------- T3 v8.6.17 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  /* ---------- T4 v8.6.17 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  ...
  /* v8.6.17 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.17"));
    return hit ? "8.6.17" : null;
  });
  gate("T9 更新日志首条 8.6.17", logFirst === "8.6.17", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.17 核心：浅色「瓷釉」重写 ----------
  ...
  /* ---------- TL9 v8.6.17 新增：掠影浅色磁贴名称白晕描边 ----------
     v8.6.15「裸文字白晕」清单漏项；且名称元素 intro 态自带 .link-intro，
     规则特异性必须 (0,3,1)（.cl-links .tile-label）才能压过 link-intro 白晕 */
  await af.evaluate(() => {
    const raw = JSON.parse(localStorage.getItem("start:settings") || "{}");
    raw.themeMode = "light";
  ...
  await sleep(2600);
  const wpLabel = await af.evaluate(() => {
    const el = document.querySelector(".cl-links .tile-label");
    if (!el) return null;
    const cs = getComputedStyle(el);
    return { ts: cs.textShadow, pm: document.documentElement.classList.contains("photo-mode"), color: cs.color };
  });
  gate("TL9 掠影浅色：磁贴名称白晕描边（压过 link-intro 同元素规则）",
    !!wpLabel && wpLabel.pm && /0(\.\d+)?(px)?\s*0px\s*3px/.test(wpLabel?.ts || "") && wpLabel.ts.includes("0.95"),
    wpLabel ? `pm=${wpLabel.pm} ts=${wpLabel.ts.slice(0, 60)}` : "no label");
  ...
  console.log(`\n===== v8.6.17 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
