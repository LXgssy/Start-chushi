// v8.7.6 视觉目检（Task 146 ㊽）：抽屉开合模糊渐入/渐出细腻化 + 掠影开抽屉背景微放大
// A 组：掠影（photo-mode）开抽屉 → wallpaper-layer scale 1→1.03 渐进放大（开抽屉体感），
//       关抽屉 → closing 失配回缩与散场 0.40s 同拍；计算值 transform 断言 + 帧序截图
// B 组：抽屉关闭散场帧序（0-35% 缓启 28→9px、35-100% linear 9→1px、hidden @0.42s——
//       渐出细腻无残留，v8.7.5 三病灶免疫架构下的长窗节奏）
// C 组：抽屉开凝聚帧序（0.36s ease-in-out 渐入——headless 帧合并下若瞬跳则 mid=final，如实记录）
import { chromium } from "playwright-core";
import crypto from "crypto";

const ROOT = "/tmp/ext-beta";
const SHOTS = "/tmp/v876-visual";
...
/* 本地 SVG 壁纸（data URI 免网络）：深蓝渐变 + 白网格线——1.03 微放大在网格
   线间距变化上肉眼可辨（3% ≈ 80px 网格变 82.4px），放大感知有视觉锚点 */
...
const ctx = await chromium.launchPersistentContext("/tmp/v876-profile", {
...
await page.screenshot({ path: `${SHOTS}/A-open-1.png` });
await sleep(110);
await page.screenshot({ path: `${SHOTS}/A-open-2-mid.png` });
await sleep(500);
await page.screenshot({ path: `${SHOTS}/A-open-3-steady.png` });
...
await page.screenshot({ path: `${SHOTS}/A-close-1.png` });
await sleep(300);
await page.screenshot({ path: `${SHOTS}/A-close-2-mid.png` });
await sleep(900);
...
await page.screenshot({ path: `${SHOTS}/A-close-3-steady.png` });
console.log("A 组完成");

/* ---------- C 组：抽屉开凝聚帧序（先截帧再读值，截图强制出帧捕中间态） ---------- */
await page.mouse.click(90, 620, { button: "middle" }); /* 开 */
...
await sleep(600);
await page.screenshot({ path: `${SHOTS}/C-open-4-steady.png` });
const cv = [];
for (let i = 0; i < 6; i++) {
  const v = await af2.evaluate(() => {
...
console.log("C 开稳态 blur 值:", JSON.stringify(cv));
await page.screenshot({ path: `${SHOTS}/B0-drawer-open.png` });
console.log("C 组完成");

/* ---------- B 组：抽屉关闭散场帧序（v8.7.6 细腻化节奏见证） ---------- */
await page.mouse.click(90, 620, { button: "middle" }); /* 关 */
