// v8.6.37 探针——互切拉伸律：面板切换动画全面重写（删 v8.6.25-28 补丁链，恢复最初拉伸动效）：
...
const ZIP = "/tmp/my-project/download/v8.6.37/ChuShi-NewTab-v8.6.37.zip";
const MOCK = "/tmp/v869-mock";
const PORT = 26997;
const SHOTS = "/tmp/probe-v869-shots";
...
const browser = await withTimeout(
  chromium.launchPersistentContext("/tmp/ext-v869-profile", {
...
/* v8.6.37：抽屉形态的隐形布局克隆（.cl-layout-ghost）与真实抽屉共享全部
...
const freezeIntro = (frame, delay) => frame.evaluate(`(() => {
  ...
  /* v8.6.37：入场播完组件会摘 intro 类（根治拖拽让位重播）——按永驻语义
...
try {
  ...
  /* ---------- T3 v8.6.37 核心：三通道入场（霜感先行 + 整块模糊覆盖）----------
  ...
  gate("T3b 包裹层 rise 单通道（v8.6.37 投影退役）",
  ...
  gate("T3h 冻结@12%: 霜层 blur 凝聚中（v8.6.37 底层律：见证由 opacity 改 blur 值，1<blur<14）",
  ...
  /* ---------- T4 v8.6.37 核心：入场中途关闭 = WAAPI 并行淡出（不瞬跳）---------- */
  await af.evaluate(() => document.getElementById("dbg-frost-style")?.remove());
  await page.keyboard.press("Escape");
  await sleep(700);
  await page.mouse.click(90, 620, { button: "middle" }); // 开
  await sleep(900); /* v8.6.37：650→900——远离霜层 0.24s 结束点，qCl 首命中是常驻叶
  ...
  /* v8.6.37 断言修正：旧断言要求重开首采样 WAAPI 即取消——headless 软渲染
  ...
  /* v8.6.37：退场见证改 transition 事件（确定性）——rAF 值采样在探针高负载下
  ...
  const exitCurve = await af.evaluate(() => new Promise((res) => {
    ...
    const tick = () => {
      rows.push({
        ...
        /* v8.6.37 补丁：扩展 iframe 环境 backdrop-filter transitionend 派发不可靠
  ...
  const logFirst = await af.evaluate(() => {
    ...
    const hit = dlgs.find((d) => d.textContent.includes("8.6.37"));
    return hit ? "8.6.37" : null;
  });
  gate("T9 更新日志首条 8.6.37", logFirst === "8.6.37", `first=${logFirst}`);
  await page.keyboard.press("Escape");
  await sleep(400);

  /* ---------- TL v8.6.37 核心：浅色「瓷釉」重写 ----------
  ...
  gate("TL3 v8.6.37 磁贴统一深色釉：浅色霜层=暗雾 rgba(12,12,18,0.2)",
  ...
  /* TL13c v8.6.37：dock 面板 glass-card 磨砂在线（浅色） */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  /* TL13c1 v8.6.37：面板出生窗——content-focus 在玻璃内层（后代 filter/opacity 不触
  ...
  const birth = await af.evaluate(() => new Promise((res) => {
    ...
    const poll = () => {
      ...
      if (card) {
        /* v8.6.37：内容层模糊聚拢在飞——blur 只存在于 0.3s 动画窗内，出生即采样
  ...
  /* TL16d/e+TL17c v8.6.37：互切拉伸律行为见证——单卡律/同卡律/零残留律 */
  ...
  /* TL16f v8.6.37 互切底锚律行为见证：壳体底缘恒定 + 玻璃卡底永不悬空 + 底锚锁在位。
    ...
    requestAnimationFrame(() => {
      ...
      const poll = () => {
        ...
        if (st && card) {
          ...
          /* v8.6.37 玻璃壳满窗律：卡高每帧==壳高（h-full 贴随弹簧，底边恒定） */
  ...
  gate("TL16f 互切底锚律 v8.6.37 玻璃壳满窗：切换窗 900ms 壳体底缘恒定（≤2px）+ 玻璃卡底永不悬空（gap≤1px）+ 卡高贴随壳高（h-full 每帧同高）",
  ...
  /* TL18a v8.6.37 雾化 filter 回归 + 玻璃载体零毒退场（静态门）：雾化 blur(12px)
  ...
  /* v8.6.37 反转：三载体禅退场=时钟同款模糊雾化（opacity:0 + blur(12px)），
  ...
  gate("TL18b' v8.6.37 模糊雾化退场：zen-gone/zen-dock/search-pill 三条 html.zen 规则 opacity:0+blur(12px)+零 scale+visibility 末帧隐没 + .zen-gone 基线 opacity/filter 过渡在位",
  ...
  /* TL20 v8.6.37 退禅磨砂复原双保险（源码门）：defrostGlass 在位（display 往返 +
  ...
  const dgCalls = (pageSrc.match(/defrostGlass\(\)/g) || []).length;
  gate("TL20 退禅磨砂复原双保险：defrostGlass 定义（display 往返+reflow+subtree 动画 cancel）+ 双退禅路径调用（dblclick/Esc）",
    /const defrostGlass = useCallback/.test(pageSrc)
      && /getAnimations\(\{ subtree: true \}\)/.test(pageSrc)
      && /offsetWidth/.test(pageSrc)
      && dgCalls >= 2
      && /if \(zenRef\.current\) defrostGlass\(\);/.test(pageSrc)
      && /defrostGlass\(\);\n\s*setZen\(false\);/.test(pageSrc),
    `defrostGlass 调用 ${dgCalls} 次`);
  /* TL21 v8.6.37 禅覆盖层常驻化（源码门）：page.tsx framer/AnimatePresence/EASE 全退役
  ...
  /* TL24a v8.6.37 dock 面板关闭收尾（静态门）：cl-panel-out-kf 材质恒定契约——
     kf 内零 background/border/box-shadow 帧（底色/描边/阴影全程自然值，不抽底色）
     + 60% 整卡 opacity:1 帧 + 82% opacity:0 帧（整体尾隐，带区前清零）+ 段内快落
     timing + blur 驻留 55%/收尾 1px 原样 + .panel-sink .cl-panel 专属挂载
     + 共享 glass-card-out-kf 原样保留（纱幕/弹窗通道零波及）。 */
  const clKf37 = (cssSrc.match(/@keyframes cl-panel-out-kf \{[\s\S]*?\n\}/) || [""])[0];
  gate("TL24a 收尾静态门：cl-panel-out-kf 材质恒定（零 bg/border/shadow 帧）+ 60%/82% 整体尾隐 + blur 语言原样 + 专属挂载 + 共享 out-kf 零波及",
    /@keyframes cl-panel-out-kf/.test(cssSrc)
      && clKf37.includes("60%") && clKf37.includes("opacity: 1")
      && clKf37.includes("82%") && clKf37.includes("opacity: 0")
      && !clKf37.includes("background-color") && !clKf37.includes("border-color") && !clKf37.includes("box-shadow")
      && clKf37.includes("cubic-bezier(0.3, 0, 0.55, 1)")
      && clKf37.includes("backdrop-filter: blur(20px) saturate(1.5)")
      && clKf37.includes("backdrop-filter: blur(1px) saturate(1.5)")
      && /\.panel-sink \.cl-panel \{[^}]*animation: cl-panel-out-kf 0\.22s cubic-bezier\(0\.4, 0, 1, 1\) forwards/.test(cssSrc)
      && /\.panel-sink \.glass-card \{[^}]*animation: glass-card-out-kf 0\.22s cubic-bezier\(0\.4, 0, 1, 1\) forwards/.test(cssSrc)
      && (cssSrc.indexOf("@keyframes cl-panel-out-kf") > cssSrc.indexOf(".panel-sink .glass-card {")),
    "材质恒定 + 整体尾隐 keyframes 在位且声明于共享规则之后");
  /* TL18b v8.6.37 磨砂底层化·图标材质：.tile-frost 材质基线在位（样式表常驻声明）+
  ...
  /* TL19 v8.6.37 禅窗零毒行为门：docked 磁贴墙在禅进出全程 opacity=1/filter=none，
  ...
  const zenPhase = await af.evaluate(async () => {
    ...
    /* v8.6.37 真实路径：进/退禅一律 dblclick 派发（React onDblClick →
  ...
  gate("TL19a v8.6.37 禅窗模糊雾化：intro 排空 + dblclick 进禅 + 磁贴墙 opacity=0+blur(12px)+visibility=hidden + 雾化 blur(12px) 在时钟段在线",
  ...
  gate("TL19b v8.6.37 退禅复原（defrostGlass 双保险）：visibility=visible + opacity=1 + filter=none + .tile-frost 磨砂 blur(14px) saturate(1.6) 恒在线",
  ...
  /* TL24b v8.6.37 收尾行为门：真实关闭链路 + WAAPI currentTime 快进（免 headless
     时间膨胀）——「底色保留 + 带区隐没」契约：t=100ms（壳高 ~72%）整卡 opacity=1
     且底色自然值（rgba alpha>0.3——v8.6.36 此刻底色已被抽成透明，本版反转断言）
     + blur(20px) 满值驻留；t=175ms（压扁带区起点，壳高 ~33%）opacity<0.1（白条/
     黑条不可见，与壁纸明暗无关）且底色仍自然值（材质恒定到隐没）；t=210ms op=0。
     时序律：关闭点击后 rAF×2 等 React 提交 closing 相位（同步采样会扑空 names=[]）；
     结束后等面板完全卸载再放行（残留面板会拦截 TL23 的 dblclick——排除表 panel!=null）。 */
  const whitebar = await af.evaluate(async () => {
    const nextFrame = () => new Promise((r) => requestAnimationFrame(r));
    const dblBtn = () => document.querySelectorAll(".dock-btn")[0].click();
    dblBtn(); /* 打开 */
    let guard = 0;
    ...
    await nextFrame();
    const card = document.querySelector(".glass-card.cl-panel");
    if (!card) return { err: "no card after close-click" };
    const anim = card.getAnimations().find((a) => a.animationName === "cl-panel-out-kf");
    if (!anim) return { err: "no cl-panel-out-kf", names: card.getAnimations().map((a) => a.animationName) };
    const snapAt = (ms) => {
      anim.currentTime = ms;
      const cs = getComputedStyle(card);
      return { op: cs.opacity, bg: cs.backgroundColor, bf: cs.backdropFilter };
    };
    const at100 = snapAt(100);
    const at175 = snapAt(175);
    const at210 = snapAt(210);
    await new Promise((r) => setTimeout(r, 1000)); /* SINK_MS+margin：面板完全卸载再放行 */
    return { at100, at175, at210 };
  });
  const bgKept = (bg) => {
  ...
  gate("TL24b 收尾行为门：t=100ms 全材质（op=1+底色保留+blur满值）/ t=175ms 带区起点已隐没（op<0.1）且底色恒定 / t=210ms op=0",
    !whitebar.err
      && whitebar.at100 && numOr(whitebar.at100.op) >= 0.999 && bgKept(whitebar.at100.bg) && /blur\(20px\)/.test(whitebar.at100.bf || "")
      && whitebar.at175 && numOr(whitebar.at175.op) < 0.1 && bgKept(whitebar.at175.bg)
      && whitebar.at210 && numOr(whitebar.at210.op) === 0,
    JSON.stringify(whitebar));
  /* TL23 v8.6.37 禅覆盖层常驻行为门：覆盖层 DOM 恒在（常驻不卸载）；dblclick 进禅
  ...
  /* TL13c2 v8.6.37：深色 glass-card 磨砂在线 */
  await af.evaluate(() => { const b = document.querySelectorAll(".dock-btn")[0]; if (b) b.click(); });
  await sleep(900);
  ...
  /* ---------- TL9 v8.6.37 重写：掠影白字统一 + 零光效 + vignette 退役 + 常驻壁纸模糊 ----------
  ...
  gate("TL9c vignette 在掠影下退役（壁纸上的顶部白光带根除）",
    ...
    `vig=${pmProbe.vigBg}`);
  gate("TL9d 壁纸滤镜退役（v8.6.37 用户指令：.photo-blur 元素不存在）",
    pmProbe.blurBf === null,
    `bf=${pmProbe.blurBf}`);
  ...
  /* ---------- TL10/TL11 v8.6.37：抽屉形态壁纸不模糊 + 时钟搜索与常驻同位 ---------- */
  await af.evaluate(() => {
    const raw = JSON.parse(localStorage.getItem("start:settings") || "{}");
  ...
  /* ---------- TL14 v8.6.37：磨砂存活 2.0 + 阴影同拍（CSSOM 扫描，稳态无关时序） ---------- */
  {
    ...
    gate("TL14c 玻璃卡退场级联在位（closing 投影规则已随 v8.6.37 退役）",
      !!cssScan.cascade,
    ...
    /* ---------- TL14g/h v8.6.37 感知同步律（blur 领先/驻留） ---------- */
    ...
    /* ---------- TL15 v8.6.37：滤镜退役/分割线同拍/散场下沉/z-48 退役/双渲染 ---------- */
    ...
    /* ---------- TL16 v8.6.37：互切单玻璃律（面板切换不再叠加两个面板） ---------- */
    ...
    /* ---------- TL17 v8.6.37：互切内容零残留律（切换不再看见上一个面板） ---------- */
    gate("TL17a 互切内容层过场在位（.cl-panel-content 模糊聚拢淡入 + .panel-sink 级联淡出——v8.6.37 模糊聚拢回归）",
  ...
  console.log(`\n===== v8.6.37 probe: ${pass} PASS / ${fail} FAIL =====`);
  await browser.close();
