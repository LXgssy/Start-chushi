...
// H 组（本版核心）：凝聚入场三路径必现——完整关闭(latch 卸载)重开/打断重开
//       animationName=dv-open-kf 在飞 + bf 渐进（v8.7.12 凝聚 animation 化：
//       transition before-change style 被 portal 重挂同帧 paint 吞 0 态根因退役）
...
/* ---------- H 组（v8.7.12 核心）：凝聚入场三路径必现见证 ----------
   修复前：完整关闭（等 latch 780ms 卸载）→ 重开 = portal 重挂，mount
   commit 与 veilOn rAF 置位 commit 在生产 BeginFrame 同一 rAF 阶段完成、
   同帧一次 paint=终态，transition 无起点瞬跳满值（用户实测「关闭动画
   完全结束后打开没有入场动画」；仅打断重开——元素常驻已 paint 过 0 态
   ——才有凝聚）。修复后凝聚 animation 化：dv-open-kf 挂上必从 from
   起播、零 paint 时序依赖，两路径统一必现。
   坑录（TL34b 同款）：visual 环境 ESC 可能只退编辑态不关抽屉——预关
   必须 stillOpen 兜底 + 关闭态确认后才发「开」click，否则 click 开关
   语义整体错位（首版 H 组拍到的是散场非凝聚，即此因）。 */
async function ensureClosed() {
  ...
  const closed = await af.evaluate(() => !document.documentElement.classList.contains("cs-drawer"));
  if (!closed) throw new Error("ensureClosed failed");
}
/* 采样器：1500ms 长窗，收 dv=1+dv-open-kf 在飞帧的 bf 序列（开凝聚段） */
function cohSampler(win) {
  return af.evaluate((w) => new Promise((res) => {
    const rows = []; const t0 = performance.now();
    const tick = () => {
      const v = document.querySelector(".cl-drawer-veil");
      if (v && v.isConnected) {
        const cs = getComputedStyle(v);
        rows.push({ t: Math.round(performance.now() - t0), bf: (cs.backdropFilter.match(/blur\(([\d.]+)px\)/) || [])[1] ?? null, anim: cs.animationName, dv: v.getAttribute("data-veil") });
      }
      if (performance.now() - t0 < w) requestAnimationFrame(tick); else res(rows);
    };
    requestAnimationFrame(tick);
  }), win);
}
const cohJudge = (rows) => {
  /* 开凝聚段：dv=1 且 animationName=dv-open-kf 的帧（散场帧 anim=scatter-kf 排除） */
  const coh = rows.filter((r) => r.dv === "1" && r.anim === "dv-open-kf");
  const bfs = [...new Set(coh.map((r) => r.bf).filter((x) => x !== null))];
  const steady = rows.some((r) => r.dv === "1" && r.bf === "14");
  return { cohN: coh.length, animInFlight: coh.length > 0, bfValues: bfs, progressive: bfs.length >= 2, lowStart: bfs.some((x) => parseFloat(x) < 5), steady };
};

/* H1：完整关闭（latch 卸载）→ 重开 */
await ensureClosed();
const h1P = cohSampler(1500);
await sleep(120);
await page.mouse.click(90, 620, { button: "middle" }); /* H1 开 */
const h1 = cohJudge(await h1P);
console.log("H1 完整重开凝聚:", JSON.stringify(h1));
console.log("H1 期望: animInFlight=true（dv-open-kf 零 paint 时序依赖必起播）+ progressive/lowStart（bf 从低值渐进；headless 帧合并时以 animInFlight 为下限证据）");
await page.screenshot({ path: `${SHOTS}/H1-reopen-cohere-mid.png` });
await sleep(900);
await page.mouse.click(90, 620, { button: "middle" }); /* H1 清场 */
await sleep(1100);

/* H2：打断重开对照（修复前就有效的路径——回归护栏） */
await ensureClosed();
await page.mouse.click(90, 620, { button: "middle" }); /* 开 */
await sleep(260); /* 凝聚中（0.5s 窗内） */
const h2P = cohSampler(1500);
await sleep(80);
await page.mouse.click(90, 620, { button: "middle" }); /* 关（打断） */
await sleep(110);
await page.mouse.click(90, 620, { button: "middle" }); /* 立即重开 */
const h2 = cohJudge(await h2P);
console.log("H2 打断重开凝聚:", JSON.stringify(h2));
console.log("H2 期望: animInFlight=true + steady=true（回归护栏：打断重开凝聚仍必现）");
await page.screenshot({ path: `${SHOTS}/H2-interrupt-reopen.png` });
await sleep(800);
await page.mouse.click(90, 620, { button: "middle" }); /* H2 清场 */
await sleep(1100);
