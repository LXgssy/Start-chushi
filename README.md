...
> v8.2.0（**悬浮音乐卡置顶所有网页 + 封面高光律动**）：两想法全落地。①悬浮迷你音乐卡出现在所有 http/https 网页——MV3 background SW 状态中继（hub 真值 1s 广播/命令代理/频谱流转发，惰性律：零卡片零轮询）+ ext-card 内容脚本（closed Shadow DOM、拖动记忆、药丸收起、按站会话级隐藏、本地插值、点卡片回面板）；chrome:// 特权页规则性不显示（诚实边界）。②封面辉光随系统音频律动——**独立频谱助手 chushi-spectrum.exe**（WASAPI loopback+FFT 16 段+bass，26911-26913）**进程内隔离 COM**（v8 宪法门不破：hub 零 COM 仅 CreateProcess+Job 监护），数据链 hub-boot→助手→smtc.ts 30Hz 包络→sandbox 双通道→部件 .cs-glow（opacity/transform 合成器友好）；暂停回落、reduced-motion 尊重、旧 hub 静态降级。测试：v820-glow 9/9 + v820-ext 9/9（真浏览器×hubsim×spectrumsim）+ 回归 77/77 + 桥 e2e 15/15。组件：NewTab v8.2.0 + cshz 8.2.0 + 桥 8.2.0（hub 8.2.0+助手 exe）必换，歌词源沿用。
> v8.1.4（**歌词高光三律根治 + 强行逐字开关 + 进度条帧率**）：用户实机四条反馈全闭环。①句尾持续高亮——宿主伪逐字时长铺满行距（lrc 行 e=下一行 s、末行 +8s）+ 部件已唱渐隐只挂切行事件；修为伪逐字按字数估算演唱时长（≈4 字/秒，下限 1.2s）+ 部件句尾渐隐律（扫光 100% 后 250ms 收尾即渐隐，不等下一句）。②回退高光残留——行切换只还原「曾 done」的行（on 行 --p 残留）+ 间奏 ref=回退前行号误标已唱；修为「当前歌曲位置之后零高光」律（未来行无条件还原）+ 宿主间奏快照携带 lastLine 已唱界。③面板新增「强行逐字歌词」开关（默认关：lrc 歌用逐行渲染；开：伪逐字准确率低；真 yrc 永不降级；storage 记忆），widgetHtmlLen 20000→22000 随宿主同步放宽。④进度条悬停放大 height 过渡（每帧 layout）改 transform scaleY 合成层动画。测试：v814-core 15/15 + v814-lyric 23/23（含旧包阴性对照）+ 回归 62 项全绿。组件：NewTab v8.1.4 + cshz 8.1.4 必换，桥/歌词源沿用。
> v8.1.3（**歌词卡死两秒闪烁根治 + 桥推送停滞根治**）：用户实机录屏「歌词还是有问题」——部件时间 1:06↔1:08 每 2 秒回跳、歌词行恒驻（肉眼即「冻住」）。取证：词扫色亮度 4fps 2.0s 周期锯齿（=熔断拒/放循环）+ 页面诊断 stateAge 11.5s（桥推送停滞）。三层根治：①桥 8.1.3 状态推送先行（/api/state 提到命令链路之前）+ 全链超时收紧（stateAge 峰值腰斩）；②sandbox.js 引擎恒源钉守——8s 窗口内重现近似拒收值=上游停滞铁证，显示封顶拒收值+0.75 原地保持，绝不向前虚构再拽回；③smtc.ts CLIENT_VER 漏升修正（8.0.9→8.1.3，诊断口 ver 如实）。测试：v813-core 18/18 + 全回归 48 项 + e2e 15/15 + 规范扩展包冒烟 10/10。
> v8.1.2（**预设包接入根治 + dock 选框连点动效根治**）：①部件 v8.1.0 防闪断宽限误写 `offSince = 0`（未声明变量，部件严格模式下每次连接渲染必抛 ReferenceError，面板永久卡「系统媒体待接入」空态——8.0.9 正常/8.1.0+ 全灭的实锢根因），修为 `offT = 0`；②dock 选框单实例化——原各按钮内条件挂载（AnimatePresence + layoutId 交接）在快速连点时新选框继承旧选框退场中的 scale/opacity 投影（逐帧取证 scale 0.44~0.59 / opacity 0.11~0.46 泵动，即用户所见「液态玻璃时期动效」），改 nav 级常驻选框 x/width 弹簧滑移，任意点击速度零交接零泵动，Q 弹出场（跃迁帧捕获）/关闭缩回/快开滑移三段语言全保留。
> v8.1.0–v8.1.1（**歌词乱跳/面板闪断双根治 + 右侧黑边根治**）：①**桥 8.1.0 位置单源化**——InfLink 时间线（SMTC 上报滞后 ~1s）与 audio 元素真值双源交替补位是歌词 1:05↔1:06 秒级锯齿的上游根源，现一律以元素 currentTime（帧级连续真值）为唯一源，InfLink 时间线仅元素缺席兜底；②NewTab 8.1.0 **核心回退熔断**（稳态期回退 0.6~6s 拍拒收、连续 2 拍放行、护航窗收窗 grace 豁免）+ seek 中间态快照保位 + smtc.ts stale 重探不闪断；③cshz 8.1.0 连接瞬断保内容黄灯 3s 防面板周期性整体消失；④v8.1.1 **右侧黑边根治**——`scrollbar-gutter:stable` 常驻 ~15px 经典滚动条槽位只露画布底色（元素与根背景绘制均裁剪在 ICB，壁纸铺不到），改根滚动条隐藏（`scrollbar-width:none`）壁纸全出血，滚动仍可用，旧 CEF 自动回退；配套 html 画布主题底色防首帧闪白。gh-pages 黑屏（EXTENSION_MODE 误部署）已重建。未变组件保留真实版本号（桥 8.1.0 / 歌词源 7.3.0 / 预设 8.1.0 / hub 8.0.9）。

> v8.0.0（**自研 SMTC 退役 · InfLink-rs 适配版**）：用户指令「InfLink-rs 插件有 smtc 功能，直接舍弃自写的 smtc，让音乐桥和 API 都去适配这个插件」——本代落地：①**ChuShi SMTC Manager 整体退役**（原生 DLL/broker/WinRT 全删，v7.0.x 四代崩溃永久终结，构建门断言零 WinRT 导入）；②**系统媒体卡片归 InfLink-rs**（第三方 Rust 插件独占持有，音乐桥零 SMTC 代码）；③**ChuShi Music Bridge 8.0.0 全新重写**——`window.InfLinkApi` 为第一真值源（getCurrentSong/getPlaybackStatus/getTimeline 毫秒时间线，播客 throw 诚实降级），缺席时五层只读阶梯补位；控制主路走 InfLinkApi（与系统卡片按钮同路 redux 派发），seek 毫秒制+读回校验；内置**零 WinRT 纯 winsock hub.dll**（127.0.0.1:26901-26903，CORS+PNA）承接页面数据通道（CEF 无法监听端口的结构性必需件）；④smtc.ts v8 同面换源：公开面字段级兼容，`smtcVer` 字段 v8 语义 = InfLink-rs 版本，枢纽身份 `chushi-music-hub`、版本门 8.0.0；⑤预设部件文案同步（安装指引/页脚 InfLink-rs 版本芯片）。验证：插件门 31（含 hub.dll 导入表零 WinRT 断言）+ e2e 39（mock 枢纽真客户端 + 桥 vm 白盒 InfLinkApi 全链 + v7 老身份否定门）× 2 轮全绿。**升级三步**：删旧 SMTC-Manager + 换 Music-Bridge 8.0.0 并完全重启网易云 → 扩展/预设更新 → InfLink-rs 保持启用。
>
...
> v5.0.0 起（**预设包与音乐面板全部相关代码从零重写：五层 0 复用达成**）：v4 完成插件/引擎/宿主重写后，用户指认「预设包和音乐面板所有相关的 API 也不用重写了？全部重写」——本代把剩余层全部归零重写：①**预设包音乐部件**（music-widget.html 全新实现，防位移三律/歌词高度迟滞/乐观翻转/芯片诚实归因全保留，视觉样式不变）；②**沙盒音乐核心**（sandbox.js `__chushiMusicCoreV5`：解析/插值/slew 0.35s 吸收/暂停按词剩余算 fadeMs，宿主与部件双通道同源零漂移）；③**新标签页数据面**（smtc.ts v5：单真值直显 1s 轮询 127.0.0.1:26801，引擎/插件版本活源三分，seekAck 诚实提示）；④**部件通道**（PresetWidgets v5 路由）；⑤**双插件 5.0.0 + 引擎 5.0.0**（构建门 ASCII 逐字符断言延续）。验证产线全面换代：verify-v5 三套（静态+白盒+e2e）**184 项断言 × 2 轮全绿**，e2e 升级为真执行渲染（iframe srcdoc 部件脚本实际运行 + sandbox ?mode=widget 生产同形加载，断言到具体渲染文案/进度推进/seek 到达/芯片四态）。**升级（三步）**：卸旧插件 → 装 ChuShi-SMTC-Manager-5.0.0.plugin + ChuShi-Music-API-5.0.0.plugin → 完全重启网易云 + Ctrl+F5 → 重导 初始SMTC音乐预设.cshz。

> v4.0.0 起（**音乐链路全量重写：插件/引擎/宿主/部件四层从零新写，零复用老代码**）：按用户三条硬性指令落地——①**彻底摆脱网易云自带 SMTC**（开关开或关都不影响）：新引擎 chushi-smtc-engine.ps1 自注册满血 Windows 媒体会话（AUMID ChuShi.SmtcEngine，媒体键/悬浮窗卡片/可拖进度/锁屏），代码零读取外部 SMTC 会话（构建门封禁 reader-side 类）；②**插件全英文**（ChuShi-SMTC-Manager-3.0.0.plugin / ChuShi-Music-API-3.0.0.plugin，文件名/manifest/代码逐字符 ASCII 断言，根治中文名读取不了）；③**老代码零复用**：宿主删除全部仲裁/守卫/锚定（judgeNcmOwns/harmonize/seekHold 等），改为单真值直显——插件只读网易云（原生事件+只读 store+元素校验，绝不改写播放进度/拦截内部调用，"装插件弄坏网易云自家进度条"结构性根治），控制只在用户主动时单次执行（元素 play/pause/currentTime 直写一次+读回校验+seekAck 诚实提示，next/prev 走网易云自家按钮）；逐字歌词按用户管线落地：eapi 全量 yrc（自实现 MD5+AES-128-ECB 过 FIPS-197 向量）→真值时间轴对齐（slew 0.35s 吸收抖动）→暂停按当前词剩余时长算 fadeMs（120-420ms）→恢复零累积漂移；端口迁移 20754→26801（旧残留引擎自动清理）。验证：verify-v4 141/141 × 2 轮（静态门+PS1 真语法门+插件 vm 白盒+音乐核心单测+mock 引擎 e2e）+ 密码学向量门。**升级**：卸载旧插件 → 装双新 .plugin → 完全重启网易云 + Ctrl+F5 → 重导 .cshz（详见交付包使用说明）。

> v3.1.0 起（**满血版 SMTC 重写：桥自带自有媒体会话 + 控制全回路 + 逐字歌词防漂移**）：按用户指令「不要依靠网易云自带的 smtc，直接自己写一个满血版的 smtc 插件，让音乐桥去连接自己写的 smtc 插件」落地。①**桥 v3.0.0 满血自有 SMTC 会话**——MediaPlayer + 官方手动控制模式（CommandManager.IsEnabled=false，`GetForWindow` interop 在 .NET SDK 受保护不可走的实证后选型）：静音内存 WAV（InMemoryRandomAccessStream，中文路径防御）注册真实系统媒体会话，AUMID='ChuShi.SmtcBridge'，IsPlaybackPositionEnabled=true + Min/MaxSeekTime → **Windows 悬浮窗/锁屏进度条真实可拖**，时间线 1Hz 墙钟推进，DisplayUpdater 元数据/封面（https URI）随插件真值驱动，真值断供 6s 自动 Closed 防僵尸卡片；网易云自带的残疾 SMTC（position 不动/seek 静默忽略）从此不承担任何角色；②**控制全回路**——媒体键/悬浮窗按钮经 Register-ObjectEvent（-MessageData 同步队列，规避 scriptblock 强转 WinRT 委托的 runspace 崩溃）→ 主循环出栈 → Try*Async 直控网易云会话，失败自动转插件队列；**悬浮窗拖动（PlaybackPositionChangeRequested）→ 插件页内 seek 阶梯**（网易云唯一接受的路径）+ 自有时间线乐观重锚（悬浮窗条即时跟手，心跳纠偏）；命令队列化（单槽改 ArrayList，面板 seek/悬浮窗按钮/拖动可并存）；桥控制门修复：无 SMTC 会话（虚拟曲目场景）时面板播放/暂停/拖动自动转插件执行（旧版 ok:false 死路），拖动的旧标题匹配门废除（真机「拖了没反应」的桥侧根因）；③**插件B v2.2.0**——控制执行器（锁定元素 play/pause 优先 + 页脚可见按钮多候选兑底，toggle 按快照播放态定方向）；seek 直写后 1600ms **末级重写**（防本体重置元素 currentTime）+ **身份捕获**（被直写且真实生效的元素立即 elLock 上锁，真值读取不再依赖评分漂移）；④**逐字歌词防累积漂移（渲染层落地用户思路）**——API 插件全量 yrc（eapi 三层回退既有）→ 真值时间轴对齐 + slew 微抖吸收（播放中 ≤0.35s 的真值抖动不重锚，逐字扫色不再肉眼抖动；大跳/播放态翻转/seek 立即重锚，显示层平滑不改真值）→ **暂停时按词时间轴计算淡入淡出时长**（clamp(120..420ms, 当前词剩余)，淡出收敛在本词边界内，恢复沿用该值淡入；st.fadeMs 由沙箱核心 calcFadeMs 计算，部件仅消费，CSS 布局零改动）；⑤**宿主 v3.1.0**——组件阈值更新（API 插件 ≥2.2.0 / 桥 ≥3.0.0），芯片文案与真实可行操作一一对应；桥 /api/state 新增 smtcOwn 诊断字段（宿主前向兼容）。verify-v31 **58/58×2 轮**（新增 PSX 真 PowerShell 解析器语法门 + ST19a/b/c 满血 SMTC 全要素 + ST20 控制门 + PB6/PB7 控制执行器白盒 + N7b 新阈值 e2e；ST/PB/PA/N 系列 v3.0.1 全量回归保留）。**升级（三步）**：更新扩展 ChuShi-NewTab-v3.1.0.zip + 卸旧装双新插件（初始SMTC桥-2.1.0.plugin 内嵌桥 3.0.0 + 初始网易云API-2.2.0.plugin）→ 完全重启网易云（桥自动升级：部署→杀旧→拉起）。

> v3.0.1 起（**四症状定点根治：逐字歌词坏 / 播放状态不同步 / 进度条不动 / 数字时间不变**）：真机反馈四症状全现 = **网易云真值断供/失真**——面板拿不到在动的真值时回退 SMTC-only，而网易云桌面版 SMTC 的 TimelineProperties position 基本不更新（这正是当初自写网易云 API 的原因），于是全冻结。三处根治（架构不动、样式不动）：①**宿主虚拟曲目兜底**——judgeNcmOwns 在桥无 SMTC 会话（track=null，真机常见：新版网易云 SMTC 注册变化/会话被抢占）时 ne 新鲜即独占，apply 以插件真值构造虚拟曲目——旧版此处 `return ne.playing` 而 apply 只认 track 非空 → 真值被判独占却无人消费 → 回退 SMTC-only 全冻结（四症状根治点一）；②**插件B v2.1.0 播放态物理自愈**——PlayState 事件丢失/store 迟到时以物理事实为准「进度在推进 = 在播放」：真值位置较上拍上报推进 >800ms 且无本端 seek → 输出 playing=true（<800ms 不误判防暂停微抖；写回 lastPlaying 由事件语义接管，真暂停时 store 交叉自愈 3s 内纠回）——面板不再显示暂停实际在响、进度/时间/逐字歌词不再因锚点 playing=false 插值恒 0 全停摆（根治点二）；③**插件B store 次级真值**——原生 PlayProgress 事件死 + 元素身份不符时用 dva playing.position（网易云自家进度条同源，秒；单位闸 (0,36000) + 时长闸防字段形态漂移）兜底，进度不再冻死 0/旧值（根治点三）。verify-v3 44/44×2 轮（新增 ST17/ST18 静态 + PB4 物理自愈三拍白盒（<800ms 不误判/>800ms 必自愈）+ PB5 store 兜底白盒（webpack require.c 同构 mock）+ N10/N10b 虚拟曲目 e2e（桥无 SMTC 会话时面板显示 ne 真值且进度推进））。**升级（两步）**：更新扩展 ChuShi-NewTab-v3.0.1.zip + 网易云插件换 初始网易云API-2.1.0.plugin（SMTC桥 2.0.0 不用动）→ 重启网易云。

> v3.0.0 起（**双插件架构重写：SMTC 与网易云 API 冲突的结构性根治**）：用户指认「smtc 和自写的网易云 api 冲突了」成立——真机九轮故障（播放状态概率反转 / 进度 0.5x 爬行倒退 / 冻死 0:00 / 版本误报旧版）的共同结构根源 = **同一份进度真值被三层各自修正互相打架**（插件 buildSnapshot 熔断 → 桥 ne-anchoring 再改 → 宿主 harmonize/零值守卫/绝对锚定再改），且一体化插件「既管桥进程又产状态」让版本误报与状态扰动互相伪装。v3.0.0 按「每数据单主」重写：①**拆成两个职责单一的插件**——「初始SMTC桥」(cc.chushi.smtcbridge v2.0.0，桥进程生命周期唯一管理者：部署读回校验/直启 powershell/杀旧双路径/独立退避/bridgeBlocked，向桥 /api/plugin/register 活体注册版本) 与「初始网易云API」(cc.chushi.ncmapi v2.0.0，网易云真值唯一生产者：原生事件+锁定元素+seek 三级阶梯全套熔断原样保留，心跳携带 role=ncm)；②**桥 v2.0.0 纯传输化**——删除 Update-MediaState 内的 ne-anchoring（三层互打的桥层终结），新增插件注册表（/api/state.plugins.smtc 活体窗口 90s）与 role 心跳仲裁（role=ncm 在场 10s 内压制旧一体化插件 role-less 心跳，并存不互打）；③**宿主单主仲裁**——judgeNcmOwns 唯一判定点：会话身份用桥 app 字段（AUMID 归一 "NetEase Music"，比标题模糊匹配可靠，标题匹配只作旧桥兜底），NCM 在场→插件真值独占（元数据/进度/时长/播放态一次性年龄补偿，零守卫零混合），不在场→SMTC-only（harmonize 守卫链只服务此路径）；宿主零值两击守卫删除（插件已有全套熔断，宿主再叠守卫=互打复辟）；NCM 正在播放时无条件独占面板（修「面板显示其它应用/暂停态而网易云在响」的反转）；④**版本误报根治**——版本各查各的活源（桥版本 /api/state.version、API 插件版本 ne.v、管理插件版本 plugins.smtc 注册表），needsBridge 新增「管理插件已注册但桥版本旧」= 旧桥进程杀不死实锤态（给手动指引绝不喊更新 .plugin）；面板芯片四态文案（缺件/旧一体化迁移/桥未运行/旧桥杀不死）与真实可行操作一一对应；⑤**预设包 v4**——样式与交互零改动（双SVG同圆心交叉淡切防播放键位移、lyHold 高度迟滞、乐观翻转、拖动失败醒目芯片全部保留），仅文案映射新架构 + 页脚双版本诊断（API vX + 管理 vY/手动桥）。**升级（三步）**：卸载旧「初始歌词源」→ 装 初始SMTC桥-2.0.0.plugin + 初始网易云API-2.0.0.plugin → 重启网易云；verify-v3 38/38×2 轮（ST16 静态 + PB/PA 插件 vm 白盒职责单一律 + e2e 单主反转实证/芯片四态/SMTC-only 兜底）。

> v2.3.2 起（**真机第 9 轮三联修复：误报旧版 / 状态反向 / 进度慢爬倒退**）：①**「插件是新版却提示旧版」根治**——真机取证（用户录屏逐帧）：页脚明明「插件 v1.4.0」在场、「组件过旧」芯片却常亮，真因 = 策略拦截下新版桥拉不起、**旧桥进程永生占端口**，宿主只见旧桥于是永远 needsUpdate，而旧文案「更新 .plugin 即可全自动修复」对这一场景是彻头彻尾的无效指引（用户照做无效直接引爆怒气）。三段修：(a) 插件 v1.5.0 新增**杀旧桥通道**——监督检测「桥可达但版本旧」→ 部署最新内嵌桥 → `cmd.exe netstat+taskkill /F /T`（cmd 常不在拦截名单）→ powershell `Get-NetTCPConnection+Stop-Process` 兜底，杀掉端口上的旧监听进程腾空端口再拉起，升级失败独立退避（30/60/120s）且 ≥2 次失败置 bridgeBlocked；(b) 宿主 v2.3.2 **升级归因拆分**——needsPlugin（插件缺失/过旧，更新 .plugin 确实能自修）与 needsBridge（插件已新但桥旧，自动升级可能被策略拦）分开判定、分开进广播签名；(c) 部件芯片**文案分叉**——仅桥旧时改口「桥组件待升级（自动升级被系统拦截）·请在压缩包内「手动启动桥（备用）」文件夹双击 启动桥.bat」的诚实手动指引，**绝不再喊无效的「更新 .plugin」**；sandbox.js snap 白名单同步补透 needsPlugin/needsBridge（真机 NU3 实锤漏透致部件拿不到归因位）。②**播放态概率反向根治**——v2.3.1 的「原生事件 5s 新鲜窗」是范畴错误：PlayState 是**事件语义**（状态切换瞬间触发一次）不是遥测流，暂停超 5s 必然「过期」降级到 `el.paused`，此时评分选到流浪/预加载元素即报反状态（录屏实锤：One Last Time 播放中面板显示 ▶ 长达 12s）。插件 v1.5.0 改 **playing 最后事件语义**（最后一次说的就是现状，永不过期；仅从未收到事件才降级元素）+ **store.paused 交叉自愈**（dva store 与原生事件矛盾持续 >3s 以 store 纠正，兜事件丢失）+ 插件加载时以 store 预热初始播放态（消除加载前已播放的空窗）。③**进度 0.5x 爬行/倒退根治**——录屏实证 34s 只爬 5s、0:09→0:08 倒退，根因 = v1.4.0「每拍重新评分」在 DOM 波动时换人读到流浪元素陈旧 currentTime 的锯齿形态。插件 v1.5.0 两闸：**元素身份锁定**（与原生期望连续对齐 ≥2 拍上锁，同歌内不换人，detached/连续脱钩 ≥3 拍才解锁，锁定期间其余元素无视）+ **倒退熔断**（同歌 + 播放态未变 + 无本端 seek，较上拍上报值倒退 >2.5s 的样本丢弃、沿用上拍值外推；song 身份不明时不熔断——宁可放过跳帧也不误杀真实新歌开头）。另加 PlayProgress 存活自检（10s 无事件重注册一次）。**桥 v1.7.1 零改动**（不给「必须升级桥」再造理由）；verify-v232 43/43×2 轮（新增 P1-P7 插件 vm 白盒：最后事件语义/倒退熔断/身份锁定/杀旧双路径实测，NU1-4 升级归因四态文案断言）；**交接文档 `docs/AI-HANDOFF.md` 落库**（组件拓扑/构建产线/兼容矩阵/八坑清单/下一步任务 A-E，供后续 AI/开发者直接接手）。**升级**：插件换 初始歌词源-1.5.0.plugin + 完全退出重启网易云 + Ctrl+F5/扩展 2.3.2 + 删除重导 .cshz；若芯片提示「手动启动桥」按说明运行备用 bat。

> v1.9.0 起（**逐字歌词 + 弹出面板重构批**）：①**逐字歌词（卡拉 OK）**：官方音乐面板新增逐词扫色歌词区——新增**「初始歌词源」BetterNCM 插件**（`bridge/lyric-plugin/`，纯 API 无 UI，可选安装）从网易云客户端内部读取逐字歌词（eapi song/lyric/v1 的 yrc 逐字 / klyric / lrc 三层回退），经 SMTC 桥新通道（`/api/plugin/state`、`/api/plugin/lyric`、`/api/lyric`）转交宿主；歌词与 SMTC 时间戳对齐（桥不可达/插件未装时歌词区自动收起，其余功能不变）；当前行下方显示翻译（ytlrc/tlyric）。②**进度条根治**：修复 seek 后进度条弹回的宿主缺陷——完整快照广播签名不含 position，seek 后新位置永远不会到达部件（v1.8.x「进度条完全是坏的」真机根因）；v1.9.0 起新增**每拍轻量锚点通道**（`onTick`：position/duration/playing/rate/fetchedAt 每拍必达，与签名无关，不携歌词大载荷），部件另在 seek 提交成功后本地乐观重锚；③网易云增强合并：SMTC 时间轴缺失（网易云 Win32 常见）时自动用插件帧级进度兑底，SMTC 封面缺失时用网易云封面 URL 兑底（面板封面三重保障：SMTC→网易云→默认唱片）；④**弹出面板重构（根治白屏）**：dock 弹出面板 iframe 由「每次打开冷加载」改为**随页面常驻预热**——打开零白屏零重载，SMTC 订阅/歌词/封面后台持续更新，弹出动画与内建面板同一套动效档位弹簧（随设置里的动效语言档）；⑤扩展 manifest 补上 `http://127.0.0.1:20754/*` host_permission——此前扩展版所有桥请求被拦，音乐面板在扩展里完全离线；⑥部件修复 `--ez` 自引用无效声明（跨 iframe 拿不到宿主变量，全部过渡静默退化）与进度条 thumb 初始定位。
> v2.1.0 起（**四联修复批**，源自用户第 6 轮演示视频逐帧取证）：①**播放/暂停按钮轻微位移根治**——双 SVG 改同圆心绝对堆叠 + opacity/scale 交叉淡切（布局零位移，帧级实测 dx=dy=0），播放三角实心化与暂停条视觉质量一致；②**暂停再继续歌词对不上根治**——网易云插件的 el.currentTime 是帧级真值，桥 v1.5.0 在插件在场且曲目匹配时用真值直接锚定进度（暂停/恢复的检测滞后不再累积成永久偏移，无插件场景另加半窗补偿）；③**自动切歌歌词概率加载不出根治**——三层孪生竞态一并拔除：宿主 fetchLyric latest-wins 链式补拉（inflight 窗口切歌不再静默丢弃）+ 重试不再被「已带旧词」抑制 + 沙箱按歌词载荷对象引用重解析（旧 parsed 布尔被切歌快照的旧词消费后真词永久跳过）+ 部件歌词键加行数组引用维（同 rev 下旧词→新词不再漏重建）+ 桥 /api/lyric 对旧词请求明确拒绝（rev-mismatch）；④**进度条拖不动根治**——网易云实测谎报 IsSeekAvailable=false，桥不再一票否决、直发 TryChangePlaybackPositionAsync 取真实返回值，同时 seek 命令经插件心跳应答直写 el.currentTime（SMTC 拒绝也跳得动，插件下一拍回报真值自动验证）。**升级四件套**：桥换 v1.5.0 + 插件换 v1.1.0（seek 直通）+ 删除重导 .cshz + Ctrl+F5。

> v2.0.1 起（**六联体验修复批**，全部源自用户第 5 轮演示视频逐帧取证）：①**暂停/恢复归零根治**——网易云 SMTC raw Position 整首钉死 0，桥 v1.3.0 在 playing 状态变化时重置锚点到 raw → 暂停瞬间 1:04→0:00、恢复后从 0 重数（录屏实锤）；桥 v1.4.0 改为「曲目未变时锚点重置用上一拍连续位置」（暂停=无缝冻结，恢复=无缝续接），宿主 smtc.ts 再加 harmonize 守卫（暂停冻结/恢复续接/持续偏移保持/本端 seek 保持 4s），旧桥不升级也全对。②**seek 全链修复**：桥 seek 命令成功后立即重锚（网易云不刷新 SMTC 时间轴也生效）+ 部件拖动预览不再被 rAF 覆写（真机「拖不动」的直接观感）+ 宿主 seekHold 顶住旧基準不被拽回。③**播放/暂停按钮响应**：宿主 control 成功后 80ms 补拍 + 部件乐观翻转（<250ms 图标即变，真实态随后确认/纠正）。④**开面板闪白根治**（CDP 帧序列取证：visibility/opacity/transform 隐藏过的 iframe 重激活首帧被合成器填纯白，且来自旧层树、DOM 层叠拦不住）——三件套：部件视图 content-focus-solid（无 opacity 的实体模糊聚拢，暗卡不再在明亮壁纸上透灰白）+ 部件壳体免 panel-rise 淡入 + sandbox 文档 color-scheme 随主题（预绘制帧暗色化）+ boot 同色罩驻留 280ms 后揭开。⑤**歌词逐字扫色闪动根治**：`-webkit-background-clip:text` 渐变在 transform 过场中部分帧不绘制（30fps 录屏实锤整词隐形）→ 双层实体色 + clip-path 裁剪（底层永可见，上层按 --p 显色）。⑥**关闭箭头朝下**（面板在 dock 上方，收起方向向下）+ **互斥单帧化**：switchTo/gotoPanel 同批清 dockWidget（原先靠 effect 二段渲染，两帧间隙双选框同 layoutId 共存 → 音乐→内建走了两段式开/关而非滑移拉伸）。**升级三件套**：桥换 v1.4.0 + 删除重导 .cshz + Ctrl+F5。

> v2.0.0 起（**统一面板舞台 + 核心内建音乐引擎批**）：①**预设面板接入切换动画**：dock 音乐面板并入内建 PanelStage 统一舞台——旧形态是「内建面板淡出 → 空档 → 音乐面板独立弹出」两段式动画互不衔接；现在开/关/**互切**与内建完全同一套语言（panel-rise/sink + content-focus/view-exit **模糊聚拢/散场** + 高度/宽度 px 弹簧同一动效档位），音乐面板 ↔ 内建面板切换是**一次连续形变**（同一舞台节点，verify 实证跨切换壳体不重挂、部件 iframe 不重载）。②**核心内建音乐引擎 `chushi.music`**（用户指令「检测 SMTC 写在初始里面，预设只写样式，用户零计算」）：歌词解析（yrc 逐字/lrc + 双语翻译对齐）、时钟插值、逐字时间戳对齐全部在宿主完成，预设/脚本直接消费预计算结果——`snapshot()/subscribe()` 给离散快照（含解析好的歌词结构），`now()` 每帧返回 `{position, progress, lineIndex, wordIndex, wordProgress, lineText, lineTr, wordText}`，`seek(s)` 成功自动乐观重锚；旧 `chushi.smtc` 保持兼容。③**进度条冻结根治（真机根因）**：网易云的 SMTC Position 整首歌不上报（用户录屏实证钉死 0:00）——桥 v1.3.0 在源头做 LastUpdatedTime 时钟补偿（raw Position/LastUpdatedTime/Playing 任一变化重置墙钟锚，其间按墙钟×速率推进），宿主 smtc.ts 对旧桥再做锚点保持兜底（曲目/播放态/速率/位置全未变则保留上拍锚点，插值得以持续前进）；verify-v200 新增跨轮询采样回归（旧实现必回跳，新版单调前进）。④打开/关闭**模糊效果**补齐（壳 panel-rise/sink + 部件 content-focus 级联模糊散场，与内建面板同词汇）。⑤**预设包瘦身**：官方音乐预设 v3 只剩样式与渲染（12875 字符 ≤18000），全部数据来自 `chushi.music`。⑥预热容器随舞台并入 Dock（`.cl-dockwidget` 移居统一舞台内，`html.zen` 隐藏规则不变）。**升级三件套**：桥重装 v1.3.0（Delivery 包）+ 删除重导新 .cshz + 网页 Ctrl+F5（旧 .cshz 的部件在 v2.0.0 宿主上仍可运行，但建议换新以获得零计算歌词）。
...
>
> v1.8.2 起（**dock 音乐按钮 + 弹出面板批，按用户指令「不是磁贴，放 dock 里」**）：①预设 widgets 新增 **`surface` 表面字段**（v1.8.2 API 扩展）：`"dock"` 表面不在角落出磁贴，而是在底部 tab 栏注册按钮（`icon` 支持内置图标名或 data:image），点击在 dock 上方**弹出同源沙箱面板**——高度弹簧（420/34）+ panel-rise/panel-sink 与内建面板同一动效语言，再点按钮 / 点击外部 / Esc / 部件内新 API `chushi.close()` 均可关闭，与内建面板互斥；dock 形态下沙箱置 `html[data-panel="1"]`，部件据此直开展开卡；②**官方 SMTC 音乐预设改用 dock 表面**：安装即见 tab 栏「音乐」按钮（Music2 图标），点开即 v1.7.x dock 面板样式的弹出音乐卡（340×248），空态 92px 随媒体接入弹簧长开；③widget 新 API `chushi.close()`（关闭 dock 弹出面板）与 `.cl-dockwidget` 动画钩子；④修桥启动 .bat 编码（第二轮）：v1.8.1 的 GBK 版在部分机器仍报 `'ANSI'/'桥'/'MTC'` 假命令——根因是 **LF（Unix）换行**使 cmd 解析器在多字节字符处行偏移错位，v1.8.2 起 .bat 以**纯 ASCII 内容 + CRLF 换行 + 无 BOM** 发布（三重保险，任何代码页/解析器下行为一致），窗口提示改英文（中文说明见使用文档）；⑤修复音乐部件空态两处视觉缺陷：`<svg width=0 height=0>` inline 行盒把卡片顶出 19px 白条（svg 改 absolute）、空态层与紧凑条同屏叠加（mode-em 下隐藏紧凑条）。
