...
> v8.2.1（**浮窗三态 + 完全体逐字/逐行歌词 + 频谱日志可发现**）：用户实机三条反馈全闭环。①spectrum-log.txt 找不到根治——助手日志原写 exe 旁，网易云插件目录常在 Program Files 下普通权限写不进且静默；改带回退链（exe 同目录试写失败→`%LOCALAPPDATA%\ChuShi\`）+ 启动首行自证实际路径（`[boot] log file (档位): 路径`）。②浮窗三态重构——**封面收起态**（整卡只显示封面+播放绿点，单击展开，旧药丸退役自动迁移）/ **标准态**（右上角 × 退役→[收起成封面][放大到完全体]，点进度条=展开完全体不再跳「初始」）/ **完全体**（与面板音乐卡同级：yrc 真逐字扫光 + lrc 逐行双模式，句尾渐隐/回退还原/间奏灰/翻译行/暂停淡出全律随行 + 时间显示 + 可 seek 进度条；歌词经 SW 代理 hub /api/lyric，songId 归属强校验不串歌；引擎独立 ext-lyric.js 与 sandbox.js 同语义，build 拼接注入）。③封面禁拖——封面纯点击目标，拖动把手只在主体空白（防拖窗口误触）；右键卡片=按站隐藏（接替 × 位）。测试：v821-lyric 36/36（解析/伪逐字估算/二分对齐/间奏 lastLine）+ v821-ext 15/15（真浏览器 e2e：closed shadow 外部探测判态 261/321/45 宽度带 + 封面禁拖 949→949 + 把手 949→868 + hublog 歌词取证 + 截图终审）+ 全回归 9 套全绿。组件：NewTab v8.2.1 + 桥 8.2.1（助手 8.2.1 日志回退）必换，cshz/歌词源沿用。
