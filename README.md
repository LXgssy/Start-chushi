...
- **预设**：声明式 JSON 预设，可给指令面板注册新命令、给底部栏加自定义按钮、批量导入磁贴；进阶玩法还可在预设里携带**沙箱 JS 脚本**（唯一源隔离，拿不到页面数据），写自己的数据源与通知机器人；**液态玻璃效果**（`effects.glass`，v1.1.0 起）一键把全站磨砂玻璃切换为背景折射的液态玻璃；一段 JSON 复制给朋友，导入即用
- **明暗主题**：跟随系统，也可手动切换
...
在页面任意空白处右键，弹出的不是浏览器默认菜单，而是「初始」专属快捷菜单：指令面板 / 添加链接 / 明暗切换 / 禅模式 / 设置 / 导出备份，常用的动作都在指尖最近处。

- 菜单是一块小玻璃卡片，随液态玻璃预设一同折射，视觉与全站同源；
- 右键输入框、文字选区时保留浏览器原生菜单（复制 / 翻译 / 拼写检查是系统级能力）；
- 靠近屏幕右 / 下缘时菜单自动翻转展开；`Esc`、外点、点击菜单项即关闭。

## 预设开发者文档

⌘K →「导入预设」→ 右上角「开发者文档」：面向预设作者的完整指南，覆盖预设 JSON 全部字段、白名单 action、沙箱脚本 API、角落小部件、`.cshz` 预设包结构与安全模型、调试与分发建议——字段与上限均与实现严格同步，照着写即可发布自己的预设。
...
| `effects` | 声明式视觉效果：液态玻璃等（v1.1.0 起，宿主内建引擎，预设不携带任何代码） |
...
### 视觉效果（`effects`，v1.1.0 起）

`effects` 字段是**声明式视觉效果开关**：预设只声明「开什么、开多大」，实际渲染由起始页**内建引擎**完成——预设不携带任何代码，参数全部白名单夹紧，**装了即生效、删除预设即还原**（与 `layout` 同律）。首个引擎是**液态玻璃**（liquid glass）：
...
| 字段 | 取值 | 说明 |
|---|---|---|
| `refraction` | 0–1.5，缺省 0.6 | 折射强度：边缘透镜弯曲的位移上限系数 |
| `bezel` | 0.2–0.7，缺省 0.5 | 边缘折射区占比：越大边缘弯曲带越宽 |
| `blur` | 0–20，缺省 6 | 折射层叠加的背景模糊 px |
| `saturation` | 80–300，缺省 170 | 折射层叠加的色彩饱和 % |

开启后，搜索药丸、底部栏、dock 面板、指令面板等全站玻璃容器从普通磨砂切换为**液态玻璃**：背景经 SVG 位移贴图实时折射（边缘像曲面玻璃一样弯曲透镜），叠加多层边缘高光与跟随指针的镜面高光，深浅色主题自动适配。官方示例「液态玻璃」预设（`examples/液态玻璃预设.json`）导入即体验。

原理与兼容性：折射由 `feDisplacementMap` 位移贴图驱动（元素尺寸变化自动重建贴图），**需要 Chromium 系浏览器**（Chrome / Edge 均可）；不支持的浏览器（Firefox / Safari）自动保持普通磨砂玻璃，不影响其它功能。多个预设都带 `effects.glass` 时按安装顺序后者胜。
