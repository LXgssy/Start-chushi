...
同一份代码，以 Chrome MV3 / Edge Add-ons 扩展形态交付：安装后**直接替换浏览器新标签页**，无需打开网址。扩展是完整的本地静态包——时钟、搜索、快捷链接、待办、便签、番茄钟、壁纸图库（含国风十帧）全部随扩展离线分发，**断网也能完整使用**（仅天气、搜索联想、在线壁纸需要网络）。

### 安装

**方式一 · Edge 商店（推荐，审核中）**

扩展已提交 Microsoft Edge 加载项商店审核，通过后此处将更新直达链接，届时在 Edge 新标签页商店搜索「初始」即可一键安装。
...
| `geolocation` | 仅在你主动点击天气「定位」时获取坐标，只用于向 Open-Meteo 查询当地天气 |
| `api.open-meteo.com` 等 | 天气实况 / 城市搜索 / 逆地理编码 |
