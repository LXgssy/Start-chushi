...
- **预设**：声明式 JSON 预设，可给指令面板注册新命令、给底部栏加自定义按钮、批量导入磁贴，甚至**向设置面板贡献自己的调节项**（v1.2.0 设置面作用面，滑杆/开关/分段选择，改动热生效）；进阶玩法还可在预设里携带**沙箱 JS 脚本**（唯一源隔离，拿不到页面数据），写自己的数据源与通知机器人；**液态玻璃**引擎内建于宿主并在可见文档中以 rAF 实时渲染（v1.3.0，几何逐帧追踪、真环绕折射），预设包经 `chushi.glass` 一句调用即换新全站玻璃材质，自定义视觉仍可走 fx 受控作用面；支持粘贴、本地文件与**拖拽文件导入**；一段 JSON 复制给朋友，导入即用
- **明暗主题**：跟随系统，也可手动切换
...
>
> v1.3.0 起，液态玻璃引擎**收编内建于宿主**（应预设包承载能力上限：沙箱隐藏文档禁 rAF、消息桥往返、贴图只能事后重建）：引擎在可见文档中以 rAF 逐帧追踪玻璃几何——**布局/弹簧/变换动画期间贴图实时重建、折射全程在线不冻结**（不再退化为纯磨砂）；物理升级为**真环绕折射**：对基础四区采用「边框外扩法」（透明边环 + 负 margin + 宽度补偿），backdrop 取样域扩到玻璃足迹之外，边缘环带折进的是玻璃外的真实世界（Apple 纸镇效应，此前受 border-box 裁剪只能拉丝边缘像素）；覆盖范围扩到全部玻璃面（新增天气玻璃芯片）并新增**覆盖范围**设置项（全部玻璃面/基础四区）。预设包改经 `chushi.glass.enable/patch/disable` 一句调用引擎，官方「液态玻璃」预设瘦身到 1.8KB 并新增第八项设置。
...
### 液态玻璃引擎（chushi.glass，v1.3.0 起）与 fx 通用作用面、设置面

液态玻璃引擎自 v1.3.0 起**内建于宿主**：引擎在可见文档中以 rAF 逐帧追踪玻璃几何，布局/弹簧动画期间位移贴图实时重建（变动期 1/4 分辨率 30fps，稳定后半分辨率精贴图），折射全程在线不冻结；物理透镜 = SDF 梯度方向 × smoothstep² 边缘窄带 × **真环绕折射**（基础四区用「边框外扩法」把 backdrop 取样域扩到玻璃足迹之外，边缘环带折进玻璃外的真实世界）。预设包只需一句 `chushi.glass.enable(cfg)` 调用引擎，参数全部白名单夹紧；单持有者制（第二个预设 enable 会收到 `ok:false`），删除预设/脚本冻结即整体回收还原磨砂。官方「液态玻璃」预设（`examples/液态玻璃预设.json`，脚本仅 1.8KB）就是参考实现：注册设置面板八项调节 + 调用引擎。

自定义视觉效果仍可走 **fx 通用作用面**（v1.1.3 起）：预设自带的自定义 `<style>`/`<svg>` 经受控通道挂载，与内建引擎互不干扰；**设置面**（v1.2.0 起）让预设向设置面板贡献自己的调节项。

| API / 钩子 | 说明 |
|---|---|
| `chushi.glass.enable(cfg)` | v1.3.0：启用内建液态玻璃引擎并持有（Promise 回执 `{ok, message?}`）。cfg 字段：`refraction`(0–300%)、`band`(8–60%)、`frost`(0–12px)、`saturation`(100–260%)、`brightness`(80–140%)、`dispersion`/`specular`(布尔)、`coverage`(`"core"`基础四区 / `"full"`全部玻璃面)；非法字段回默认 |
| `chushi.glass.patch(cfg)` | 热更新部分参数（设置面板拖动滑杆即走这里），字段同上可任选 |
| `chushi.glass.disable()` | 停用并交还引擎，全站玻璃还原磨砂 |
...
| `[data-fx="fxN"]` | 宿主打在白名单玻璃容器上的稳定标记，预设 CSS 用它触达真实元素 |
...
安全边界：mount 的 html 只接受 `<style>` / `<svg>` 顶层结构（禁 `script`、事件属性、foreignObject 与外链资源）；全屏幕布（⌘K / 对话框遮罩）永不折射、永不打标——幕布不是玻璃块。

⚠ **材质即顺序（链序律）**：`backdrop-filter` 引用 SVG 滤镜时必须 `blur` 在前、`url(#filter)` 在后（先霜化再折射，弯曲锐利）；写反了折射会被模糊糊掉。液态玻璃引擎已内置该律；用 fx 作用面自写同类效果时请照抄。需要 Chromium 系浏览器（`backdrop-filter: url()`），Firefox / Safari 自动保持普通磨砂。

⚠ **实时渲染律（v1.3.0）**：折射贴图是几何的函数——玻璃几何（含 transform 弹簧）变化时贴图必须随之重建。引擎在宿主可见文档中 rAF 逐帧追踪：变动期 1/4 分辨率 30fps 重建（折射在线）、稳定 ~160ms 后换半分辨率精贴图、新贴图经 Image 预解码后原子换 href（无空窗帧）。此前「沙箱脚本自建引擎」方案受隐藏文档禁 rAF 所限只能退化纯模糊，正是「玻璃不实时」的根源。
