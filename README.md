...
| `clock` | 时钟格式覆写：12/24 小时制、秒数、日期行、问候语模板（`{greet}`/`{name}` 占位，v1.7.0 起） |
...
不想手写 JSON？⌘K →「导入预设」→「**开发工具**」可下载官方**图形化预设开发工具**：单文件离线 HTML 应用（内嵌在应用静态资源里，不依赖任何服务器，断网也能下载使用），表单式编辑全部声明式字段、实时生成 JSON 与完整性提示、内嵌使用说明与上限速查。下载后双击在任意浏览器打开即可。
